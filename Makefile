...
prebuild:
	$(VERBOSE)for DEP in $(DEPENDENCIES); do \
		...
		$(MAKE) -C $$DEP || { echo "Error building dependency $$DEP"; exit 1; }; \
	done
...
%.o: $(MODULE_DIR)/%.c
	@echo "Compiling $< to $@"
	$(VERBOSE)$(CC) -D$(DEVICE) $(CFLAGS) -c $< -o $@ $(EXTRA)
...
$(MODULES):
	...
	if [ -f "$$UI_FILE" ]; then \
		rm -f "$$UI_OBJ"; \
		$(MAKE) -C $(MODULE_DIR)/ui ui_$@.o || { echo "Error building UI object"; exit 1; }; \
	else \
		UI_OBJ=""; \
	fi; \
	$(CC) -D$(DEVICE) $(MODULE_DIR)/$@.c $$UI_OBJ -o $@ $(EXTRA) || { echo "Error building $@"; exit 1; }; \
