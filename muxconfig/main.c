...
void show_help(lv_obj_t *element_focused) {
    ...
    } else if (element_focused == ui_lblReset) {
        message = MUXCONFIG_RESET;
    }
...
void init_elements() {
    ...
    lv_obj_set_user_data(ui_lblReset, "refresh");
    ...
    if (strlen(overlay) > 0 && theme.MISC.IMAGE_OVERLAY) {
        lv_obj_t * overlay_img = lv_img_create(ui_scrConfig);
        lv_img_set_src(overlay_img, overlay);
        lv_obj_move_foreground(overlay_img);
    }

    if (TEST_IMAGE) display_testing_message(ui_scrConfig);
}
    ...
    if (nav_moved) {
        if (lv_group_get_obj_count(ui_group) > 0) {
            ...
            snprintf(new_wall, sizeof(new_wall), "%s", load_wallpaper(
                    ui_scrConfig, ui_group, theme.MISC.ANIMATED_BACKGROUND));
            ...
            snprintf(static_image, sizeof(static_image), "%s",
                     load_static_image(ui_scrConfig, ui_group));
...
int main(int argc, char *argv[]) {
    ...
    lv_obj_set_user_data(ui_scrConfig, basename(argv[0]));
    ...
    current_wall = load_wallpaper(ui_scrConfig, NULL, theme.MISC.ANIMATED_BACKGROUND);
