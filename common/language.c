...
#define SPECIFIC_FIELD(field, string) fill_specific((string), (field), MAX_BUFFER_SIZE)
    ...
    SPECIFIC_FIELD(lang->MUXACTIVITY.STYLE.LOCAL.LONG, "Long Sessions");
    ...
    SPECIFIC_FIELD(lang->MUXNETPROFILE.STATIC, "Static");
    ...
    SPECIFIC_FIELD(lang->MUXNETPROFILE.HELP.PRIORITY, "Set connection priority (0 highest, 9 lowest) when multiple saved networks are in range");
    ...
    SPECIFIC_FIELD(lang->MUXNETWORK.AUTO, "Auto");
    ...
    SPECIFIC_FIELD(lang->MUXTHEMEOPT.HELP.GLYPHLIST, "Override the list glyph render size in pixels (Default uses the theme value, Auto fits to item height, Native uses the actual size)");
    ...
    SPECIFIC_FIELD(lang->MUXVISUAL.HELP.SELECTIONANIMATION, "Controls the intensity of the selection animation when navigating between list and grid items");
    SPECIFIC_FIELD(lang->MUXVISUAL.HELP.SELECTIONSTYLE, "Controls the style of the selection animation applied when navigating between list and grid items");
