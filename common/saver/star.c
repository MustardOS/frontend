...
static void apply_star_colour(star_t *s) {
    if (mod.base.speed >= SAVER_SPEED_COLOUR_THRESHOLD) {
        saver_pastel_pick(saver_rand_range(SAVER_PASTEL_COUNT), &s->r, &s->g, &s->b);
    } else {
...
static void trail_push(star_t *s, const int x, const int y) {
...
static void star_reset(star_t *s, const int randomise_z) {
    const int32_t spread = STAR_SPREAD;
    int32_t rx, ry;
    int tries = 0;
    ...
    if (randomise_z) {
        s->z = (STAR_Z_NEAR + saver_rand_range(STAR_Z_FAR - STAR_Z_NEAR)) << SAVER_FRAME_SHF;
    } else {
        s->z = STAR_Z_FAR << SAVER_FRAME_SHF;
    }
...
static void apply_drifter_colour(drifter_t *d) {
    if (mod.base.speed >= SAVER_SPEED_COLOUR_THRESHOLD) {
        saver_pastel_pick(saver_rand_range(SAVER_PASTEL_COUNT), &d->r, &d->g, &d->b);
    } else {
...
static void drifter_spawn(drifter_t *d, const int interior) {
    const int cx = mod.base.screen_w / 2;
    const int cy = mod.base.screen_h / 2;

    const int half_w = mod.base.screen_w / 2;
    const int half_h = mod.base.screen_h / 2;

    const int shorter = mod.base.screen_w < mod.base.screen_h ? mod.base.screen_w : mod.base.screen_h;
...
#define DRIFTER_RAND_INT(LIMIT)                                                                                        \
    ...
    for (int tries = 0; tries < 48; tries++) {
        const int px = DRIFTER_RAND_INT(mod.base.screen_w);
        const int py = DRIFTER_RAND_INT(mod.base.screen_h);

        const int dx = px - cx;
        const int dy = py - cy;

        const int nx = half_w > 0 ? dx * 1024 / half_w : 0;
        const int ny = half_h > 0 ? dy * 1024 / half_h : 0;

        const int dist = (nx * nx + ny * ny) >> 10;
    ...
    d->fx = x << SAVER_FRAME_SHF;
    d->fy = y << SAVER_FRAME_SHF;

    int dx = x - cx;
    int dy = y - cy;

    if (dx == 0 && dy == 0) {
        const int range = shorter > 1 ? shorter : 2;
    ...
    const int speed_mag = 5 + DRIFTER_RAND_INT(18);

    d->vx = dx * speed_mag / norm;
    d->vy = dy * speed_mag / norm;

    d->vx += DRIFTER_RAND_INT(7) - 3;
    d->vy += DRIFTER_RAND_INT(7) - 3;
...
static void seed_drifters(void) {
    for (int i = 0; i < STAR_DRIFTER_COUNT; i++)
        drifter_spawn(&mod.drifter[i], 1);
}

static void seed_all(void) {
    for (int i = 0; i < STAR_COUNT; i++)
        star_reset(&mod.stars[i], 1);
    seed_drifters();
}
...
static void on_speed_changed(void *user) {
    ...
    mod.z_step_per_ms = speed * STAR_Z_SCALE * SAVER_FRAME_ONE / 1000;
    mod.drifter_speed_fp_per_ms = speed * SAVER_FRAME_ONE / 4000;
...
int star_init(const SDL_Renderer *renderer, const int screen_w, const int screen_h) {
    (void) renderer;

    const int focal = screen_w < screen_h ? screen_w : screen_h;
    mod.focal = focal / 2;
    ...
    LOG_INFO(
        "saver", "Starfield Initialised (%dx%d, focal=%d, speed=%d)", screen_w, screen_h, mod.focal, mod.base.speed
    );
...
void star_update(void) {
    ...
    const int32_t dz = mod.z_step_per_ms * (int32_t) elapsed;
    ...
    for (int i = 0; i < STAR_COUNT; i++) {
        ...
        const int32_t z_world = s->z >> SAVER_FRAME_SHF;
        if (z_world <= 0) {
            star_reset(s, 0);
            continue;
        }

        const int sx = half_w + (int) ((int64_t) s->dx * mod.focal / z_world);
        const int sy = half_h + (int) ((int64_t) s->dy * mod.focal / z_world);

        if (sx < -STAR_MAX_SIZE || sx > mod.base.screen_w + STAR_MAX_SIZE || sy < -STAR_MAX_SIZE
            || sy > mod.base.screen_h + STAR_MAX_SIZE) {
            star_reset(s, 0);
            continue;
        }
    ...
    const int32_t drift_step = mod.drifter_speed_fp_per_ms * (int32_t) elapsed;

    const int margin = STAR_MAX_SIZE * 4;

    const int32_t max_fx = (mod.base.screen_w + margin) << SAVER_FRAME_SHF;
    const int32_t max_fy = (mod.base.screen_h + margin) << SAVER_FRAME_SHF;

    const int32_t min_fx = -margin << SAVER_FRAME_SHF;
    const int32_t min_fy = -margin << SAVER_FRAME_SHF;
    ...
    for (int i = 0; i < STAR_DRIFTER_COUNT; i++) {
        ...
        if (d->fx < min_fx || d->fx > max_fx || d->fy < min_fy || d->fy > max_fy) {
            drifter_spawn(d, 0);
        }
    }
}
...
void star_render(SDL_Renderer *renderer) {
    ...
    for (int i = 0; i < STAR_DRIFTER_COUNT; i++) {
        ...
        const int x = d->fx >> SAVER_FRAME_SHF;
        const int y = d->fy >> SAVER_FRAME_SHF;
    ...
    const int32_t z_near_fp = STAR_Z_NEAR << SAVER_FRAME_SHF;
    const int32_t z_far_fp = STAR_Z_FAR << SAVER_FRAME_SHF;
    int32_t z_span_fp = z_far_fp - z_near_fp;
    if (z_span_fp <= 0) z_span_fp = 1;

    const int draw_trails = mod.base.speed >= SAVER_SPEED_COLOUR_THRESHOLD;
    ...
    for (int i = 0; i < STAR_COUNT; i++) {
        ...
        const int size = 1 + (int) ((int64_t) depth_fp * (STAR_MAX_SIZE - 1) / z_span_fp);
        const int alpha = 32 + (int) ((int64_t) depth_fp * (255 - 32) / z_span_fp);

        const int sx = s->trail_x[s->trail_head];
        const int sy = s->trail_y[s->trail_head];

        if (draw_trails && s->trail_count > 1) {
            int cur_idx = s->trail_head;
            for (int seg = 0; seg < s->trail_count - 1; seg++) {
                const int prev_idx = (cur_idx + STAR_TRAIL_LEN - 1) % STAR_TRAIL_LEN;

                int seg_alpha = alpha - alpha * 7 * seg / (STAR_TRAIL_LEN * 8);
                if (seg_alpha < 8) seg_alpha = 8;

                const int x1 = s->trail_x[cur_idx];
                const int y1 = s->trail_y[cur_idx];
                const int x2 = s->trail_x[prev_idx];
                const int y2 = s->trail_y[prev_idx];
