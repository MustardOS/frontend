...
static int hex_to_byte(const char *hex, uint8_t *out) {
    const char buf[3] = {hex[0], hex[1], '\0'};

    char *endptr;
    const long val = strtol(buf, &endptr, 16);
...
int read_rgb_colour_from_file(const char *filepath, rgb_colour_t *out, const rgb_colour_t *fallback) {
    ...
    char *endptr;
    const long idx = strtol(line, &endptr, 10);

    if (*endptr == '\0') {
        // Entire string was a valid integer

        const size_t count = sizeof(rgb_colours) / sizeof(rgb_colours[0]);
