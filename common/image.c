...
static lv_res_t dims_png(const uint8_t *d, uint32_t n, lv_coord_t *w, lv_coord_t *h) {
...
static lv_res_t dims_gif(const uint8_t *d, uint32_t n, lv_coord_t *w, lv_coord_t *h) {
    if (n < 10 || memcmp(d, "GIF8", 4) != 0) return LV_RES_INV;

    *w = (lv_coord_t) ((uint16_t) d[6] | ((uint16_t) d[7] << 8));
    *h = (lv_coord_t) ((uint16_t) d[8] | ((uint16_t) d[9] << 8));

    return (*w && *h) ? LV_RES_OK : LV_RES_INV;
}

static lv_res_t dims_bmp(const uint8_t *d, uint32_t n, lv_coord_t *w, lv_coord_t *h) {
    if (n < 26 || d[0] != 'B' || d[1] != 'M') return LV_RES_INV;

    int32_t bw = (int32_t) ((uint32_t) d[18] | ((uint32_t) d[19] << 8) | ((uint32_t) d[20] << 16) | ((uint32_t) d[21] << 24));
    int32_t bh = (int32_t) ((uint32_t) d[22] | ((uint32_t) d[23] << 8) | ((uint32_t) d[24] << 16) | ((uint32_t) d[25] << 24));

    if (bw <= 0 || bh == 0) return LV_RES_INV;

    *w = (lv_coord_t) bw;
    *h = (lv_coord_t) (bh < 0 ? -bh : bh);

    return LV_RES_OK;
}

static lv_res_t dims_webp(const uint8_t *d, uint32_t n, lv_coord_t *w, lv_coord_t *h) {
    ...
    if (memcmp(type, "VP8L", 4) == 0 && d[20] == 0x2F) {
        uint32_t b = (uint32_t) d[21] | ((uint32_t) d[22] << 8) | ((uint32_t) d[23] << 16) | ((uint32_t) d[24] << 24);
    ...
    if (memcmp(type, "VP8 ", 4) == 0 && (d[20] & 0x01) == 0 &&
        d[23] == 0x9D && d[24] == 0x01 && d[25] == 0x2A) {
        *w = (lv_coord_t) ((uint16_t) d[26] | ((uint16_t) d[27] << 8)) & 0x3FFF;
        *h = (lv_coord_t) ((uint16_t) d[28] | ((uint16_t) d[29] << 8)) & 0x3FFF;
...
static lv_res_t dims_pcx(const uint8_t *d, uint32_t n, lv_coord_t *w, lv_coord_t *h) {
    if (n < 12 || d[0] != 0x0A || d[1] > 5) return LV_RES_INV;

    uint16_t xmin = (uint16_t) d[4] | ((uint16_t) d[5] << 8);
    uint16_t ymin = (uint16_t) d[6] | ((uint16_t) d[7] << 8);

    uint16_t xmax = (uint16_t) d[8] | ((uint16_t) d[9] << 8);
    uint16_t ymax = (uint16_t) d[10] | ((uint16_t) d[11] << 8);

    if (xmax < xmin || ymax < ymin) return LV_RES_INV;

    *w = (lv_coord_t) (xmax - xmin + 1);
    *h = (lv_coord_t) (ymax - ymin + 1);

    return (*w && *h) ? LV_RES_OK : LV_RES_INV;
}

static lv_res_t dims_qoi(const uint8_t *d, uint32_t n, lv_coord_t *w, lv_coord_t *h) {
...
static lv_res_t dims_tga(const uint8_t *d, uint32_t n, lv_coord_t *w, lv_coord_t *h) {
    if (n < 16 || d[1] > 1) return LV_RES_INV;

    uint8_t t = d[2];
    if (!((t >= 1 && t <= 3) || (t >= 9 && t <= 11))) return LV_RES_INV;

    *w = (lv_coord_t) ((uint16_t) d[12] | ((uint16_t) d[13] << 8));
    *h = (lv_coord_t) ((uint16_t) d[14] | ((uint16_t) d[15] << 8));

    return (*w && *h) ? LV_RES_OK : LV_RES_INV;
}

static lv_res_t dims_jpeg(const uint8_t *d, uint32_t n, lv_coord_t *w, lv_coord_t *h) {
    ...
    while (pos + 3 < n) {
        ...
        if ((marker >= 0xC0 && marker <= 0xC3) || (marker >= 0xC5 && marker <= 0xC7) ||
            ...
            *h = (lv_coord_t) (((lv_coord_t) d[pos + 1] << 8) | d[pos + 2]);
            *w = (lv_coord_t) (((lv_coord_t) d[pos + 3] << 8) | d[pos + 4]);
...
static int supports_magic(const uint8_t *d, uint32_t n) {
...
static lv_res_t dims_for_data(const uint8_t *d, uint32_t n, lv_coord_t *w, lv_coord_t *h) {
...
static uint8_t *surface_to_lv_pixels(SDL_Surface *src) {
    ...
    uint32_t row = (uint32_t) bgra->w * 4;
    uint32_t total = row * (uint32_t) bgra->h;
...
static lv_res_t decoder_info(struct _lv_img_decoder_t *decoder, const void *src, lv_img_header_t *header) {
    ...
    if (src_type == LV_IMG_SRC_FILE) {
        res = dims_for_file((const char *) src, &w, &h);
