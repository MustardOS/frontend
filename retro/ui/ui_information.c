...
void information_menu_tick(void) {
    ...
    if (pause_menu_help_input(edge & BIT(0), edge & BIT(1), menu_tap || edge & BIT(5))) return;

    if (menu_tap) {
        play_sound(snd_info_open);
        show_info_box(lang.muxretro.information, lang.muxretro.help.screen.information, 0);
        return;
    }
