...
static void build_video_row(const int index) {
...
static void video_nav(const int index, const int force) {
    const int slider_row = index != row_viewport;
...
void video_menu_init(void) {
    ...
    dialogue_init(
        &save_dlg, &theme, ui_screen, lang.muxretro.save.video_title, lang.muxretro.save.video_desc, save_options, 4,
...
void video_menu_open(void) {
    ...
    video_nav(current_item_index, 1);
}
...
void video_menu_tick(void) {
    ...
    const int cycle_allowed = current_item_index != row_viewport;
    ...
    if (do_up) {
        ...
        video_nav(current_item_index, 0);
    } else if (do_down) {
        ...
        if (current_item_index == row_viewport) {
            play_sound(snd_confirm);
            viewport_menu_open();
        }
    } else if (edge & BIT(5)) {
