...
static void build_row(const int index) {
    ...
    apply_theme_option_item_label(&theme, label, colour_filter_preset_label(index), 1);
    apply_theme_list_glyph(&theme, icon, "muxretro", "filter");
    apply_theme_list_value(&theme, value, "");
    apply_size_to_content(&theme, ui_pnl_content, label, icon, colour_filter_preset_label(index));
...
void colfilter_menu_open(void) {
    ...
    entry_value = session_settings.colour_filter;

    rebuild_rows();
    focus_item(session_settings.colour_filter);
...
void colfilter_menu_tick(void) {
    ...
    if (do_up) {
        ...
        session_settings_set_colour_filter(current_item_index);
    } else if (do_down) {
        ...
        play_sound(snd_back);
        session_settings_set_colour_filter(entry_value);
        close_screen();
    }
}
