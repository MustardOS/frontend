...
static int selected_is_removable(void) {
    return current_item_index > 0 && colour_filter_is_user(current_item_index - 1);
}
...
static void delete_selected(void) {
    ...
    if (current_item_index > 0) session_settings_set_colour_filter(current_item_index - 1);
    refresh_nav();
...
static void build_row(const int index) {
    ...
    const char *text = index == 0 ? lang.generic.download : colour_filter_preset_label(index - 1);
    apply_theme_option_item_label(&theme, label, text, 1);
    apply_theme_list_glyph(&theme, icon, "muxretro", index == 0 ? "download" : "filter");
...
static void rebuild_rows(void) {
    ...
    const int count = colour_filter_preset_count() + 1;
...
void colfilter_menu_tick(void) {
    ...
    if (do_up) {
        ...
        session_settings_set_colour_filter(current_item_index - 1);
