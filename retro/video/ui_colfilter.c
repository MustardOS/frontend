...
static int selected_is_removable(void) {
    const preset_browser_row *row = selected_row();
    return !browser.collection && row && row->type == preset_browser_row_item && colour_filter_is_user(row->item_index);
}
...
static void refresh_nav(void) {
    ...
    if (!row || row->type == preset_browser_row_empty) {
        ...
        if (browser.collection || !preset_browser_is_collected(preset_browser_filter, row->key)) {
...
static int refresh_browser(void) {
    return preset_browser_configure(
        &browser, colour_filter_preset_count(), colour_filter_preset_key, colour_filter_preset_label
    );
}
...
static void preview_selected(void) {
    ...
    if (row->type == preset_browser_row_none)
        session_settings_set_colour_filter(0);
    else if (row->type == preset_browser_row_item)
        session_settings_set_colour_filter(row->item_index);
}

static void close_screen(void) {
    active = 0;
    display_menu_reopen_filter();
}
...
static void delete_selected(void) {
    ...
    if (!colour_filter_delete(row->item_index)) {
        pause_menu_show_toast(lang.muxretro.catalogue_screen.delete_failed);
        return;
    }
    session_settings_set_colour_filter(0);
    colour_init();
    if (colour_filter_preset_index(key) < 0) preset_browser_remove_collection(preset_browser_filter, key);
...
static void toggle_collection(void) {
    ...
    char key[PRESET_BROWSER_KEY_MAX];
    snprintf(key, sizeof(key), "%s", row->key);
    const int result = preset_browser_toggle_collection(preset_browser_filter, key);
    if (result < 0) {
...
void colfilter_menu_init(void) {
    ...
    preset_browser_init(&browser, preset_browser_filter);
...
void colfilter_menu_reopen(void) {
    reopen_key(colour_filter_preset_key(session_settings.colour_filter));
}
...
void colfilter_menu_open(void) {
    snprintf(entry_key, sizeof(entry_key), "%s", colour_filter_preset_key(session_settings.colour_filter));
...
void colfilter_menu_tick(void) {
    ...
    if (do_up) {
        ...
        const preset_browser_row *row = selected_row();
        if (!browser.collection && preset_browser_is_collected(preset_browser_filter, row->key)) return;
        ...
        if (row->type == preset_browser_row_download) {
            preset_catalogue_open(preset_catalogue_filter);
            return;
        }
        if (row->type == preset_browser_row_directory || row->type == preset_browser_row_collection) {
            ...
            if (row->type == preset_browser_row_collection)
                snprintf(
                    selected_key, sizeof(selected_key), "%s", colour_filter_preset_key(session_settings.colour_filter)
                );
        ...
        const int index = colour_filter_preset_index(entry_key);
        session_settings_set_colour_filter(index >= 0 ? index : 0);
        close_screen();
    }
}
