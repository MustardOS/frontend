...
static void rebuild_list(void) {
    ...
    for (int i = 0; i < macro_count; i++) {
        ...
        build_row(macro_list[i].name, value_text, "macro");
    }
...
static void rebuild_editor_rows(void) {
    ...
    for (int i = 0; i < step_count; i++) {
        ...
        build_row(label_text, session_settings_turbo_rate_name(step->hz_rate), "macro");
    }
...
static void apply_editor_nav(void) {
    ...
    nav_show_lr(ui_count_static > 0);
    lv_label_set_text(ui_lbl_nav_lr, lang.muxretro.settings_screen.turbo_modes);
    pause_menu_fix_nav_order();
}
...
static void finish_add_step(const uint16_t target_mask) {
    int new_pos = -1;
    if (target_mask != 0) {
        new_pos = macros_add_step(editing_position, target_mask, 0);
        play_sound(snd_confirm);
    }
...
void macros_menu_init(void) {
    ...
    dialogue_init(
        &edit_bind_dlg, &theme, ui_screen, "", NULL, edit_bind_options, 2, lang.generic.select, lang.generic.cancel
    );
...
void macros_menu_tick(void) {
    ...
    if (do_up) {
        ...
        nav_unsuppress_shake();
        gen_step_movement(1, +1, 1, 0, 1);
    } else if (edit_active && (do_left || do_right) && ui_count_static > 0) {
        const struct macro_step *step = &macro_list[editing_position].steps[current_item_index];
        macros_set_step_hz(editing_position, current_item_index, (step->hz_rate + (do_right ? 1 : 3)) % 4);
        refresh_row_value(
            current_item_index,
            session_settings_turbo_rate_name(macro_list[editing_position].steps[current_item_index].hz_rate)
        );
        ...
        if (edit_active) {
            if (macro_list[editing_position].step_count < MACRO_STEP_MAX) begin_capture(capture_purpose_add_step);
        } else if (macro_count < MACRO_MAX) {
            start_new_macro();
        }
    } else if (edge & BIT(4)) {
        if (!edit_active && ui_count_static > 0) {
            play_sound(snd_confirm);
