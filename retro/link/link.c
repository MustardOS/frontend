...
static uint8_t direct_self_mac[6];
static uint8_t direct_peer_mac[6];

static int election_fd = -1;
static uint16_t election_port = 0;
static char election_address[LINK_HOST_LEN] = "";
static char election_peer_address[LINK_HOST_LEN] = "";
static struct sockaddr_in election_peer;
static uint32_t election_opened_at = 0;
static uint32_t election_next_tx = 0;
...
static void election_close(void) {
    if (election_fd >= 0) close(election_fd);
    election_fd = -1;
    election_port = 0;
    election_address[0] = '\0';
    election_peer_address[0] = '\0';
    memset(&election_peer, 0, sizeof(election_peer));
    election_opened_at = 0;
    election_next_tx = 0;
}
...
static void direct_refresh(void) {
    ...
    uint8_t self[6];
    uint8_t peer[6];
    const int paired = report_read && strcmp(report.status, "paired") == 0 && direct_parse_mac(report.mac, self)
                       && direct_parse_mac(report.peer_mac, peer) && memcmp(self, peer, sizeof(self)) != 0
    ...
    if (!paired) {
        ...
        direct_host[0] = '\0';
        direct_peer[0] = '\0';
        memset(direct_self_mac, 0, sizeof(direct_self_mac));
    ...
    const int changed = !direct_connected || strcmp(direct_address, report.address) != 0
                        || strcmp(direct_peer, report.peer_address) != 0
                        || memcmp(direct_self_mac, self, sizeof(self)) != 0
                        || memcmp(direct_peer_mac, peer, sizeof(peer)) != 0;
    if (!changed) return;

    direct_connected = 1;
    direct_paired = 0;
    snprintf(direct_address, sizeof(direct_address), "%s", report.address);
    snprintf(direct_peer, sizeof(direct_peer), "%s", report.peer_address);
    memcpy(direct_self_mac, self, sizeof(direct_self_mac));
    memcpy(direct_peer_mac, peer, sizeof(direct_peer_mac));
    election_close();

    if (direct_role != link_mode_off && !direct_set_core_mode(link_mode_off, NULL)) {
        direct_connected = 0;
        direct_paired = 0;
        LOG_WARN(mux_module, "Could not reset Game Link while the Direct Link peer changed");
        return;
    }

    LOG_INFO(mux_module, "Direct Link peer found; determining which Pickles session started first");
}
...
static int election_open(const uint16_t port) {
    ...
    election_fd = fd;
    election_port = port;
    snprintf(election_address, sizeof(election_address), "%s", direct_address);
    snprintf(election_peer_address, sizeof(election_peer_address), "%s", direct_peer);
    election_peer = peer;
    election_opened_at = SDL_GetTicks();
    election_next_tx = 0;
    LOG_INFO(mux_module, "Direct Link Pickles election opened on UDP %u", port);
    return 1;
}

static void election_configure(void) {
    const uint16_t port = election_wanted_port();
    if (!direct_pending || !direct_connected || !port) {
        if (election_fd >= 0) election_close();
        return;
    }

    if (election_fd >= 0 && port == election_port && strcmp(direct_address, election_address) == 0
        && strcmp(direct_peer, election_peer_address) == 0)
        return;

    election_close();
    if (!election_open(port))
        LOG_WARN(mux_module, "Could not open Direct Link Pickles election on UDP %u: %s", port, strerror(errno));
}

static enum link_mode election_role(const uint32_t now, const uint32_t peer_age) {
    const uint32_t local_age = now - direct_started_at;
    if (local_age > peer_age + LINK_ELECTION_START_TOLERANCE_MS) return link_mode_host;
    if (peer_age > local_age + LINK_ELECTION_START_TOLERANCE_MS) return link_mode_join;

    return memcmp(direct_self_mac, direct_peer_mac, sizeof(direct_self_mac)) < 0 ? link_mode_host : link_mode_join;
}
...
static void election_apply_role(enum link_mode role) {
    ...
    if (!direct_set_core_mode(role, host)) {
        direct_paired = 0;
        LOG_WARN(mux_module, "Could not apply the elected Direct Link role to this core");
        return;
    }

    direct_paired = 1;
    LOG_INFO(
        mux_module, "Direct Link configured Game Link as %s at %s; the first Pickles session is the host",
        role == link_mode_host ? "host" : "client", host
    );
}

static void election_receive(const uint32_t now) {
    for (int count = 0; count < 8; count++) {
        ...
        if (received != LINK_ELECTION_PACKET_SIZE || memcmp(packet, LINK_ELECTION_MAGIC, 4) != 0
            || packet[4] != LINK_ELECTION_VERSION || packet[5] > link_mode_join || packet[6] != 0 || packet[7] != 0
            || source_len != sizeof(source) || source.sin_family != AF_INET
            || source.sin_addr.s_addr != election_peer.sin_addr.s_addr || source.sin_port != election_peer.sin_port)
            continue;

        uint32_t encoded_age;
        memcpy(&encoded_age, packet + 8, sizeof(encoded_age));
        enum link_mode role = election_role(now, ntohl(encoded_age));
        const enum link_mode peer_role = (enum link_mode) packet[5];

        if (direct_role == link_mode_off && peer_role != link_mode_off) {
            role = peer_role == link_mode_host ? link_mode_join : link_mode_host;
        } else if (direct_role != link_mode_off && peer_role != link_mode_off) {
            if (direct_role != peer_role)
...
static void election_send(const uint32_t now) {
    ...
    const ssize_t sent = sendto(
        election_fd, packet, sizeof(packet), MSG_DONTWAIT, (const struct sockaddr *) &election_peer,
        sizeof(election_peer)
    );
    if (sent < 0 && errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR)
        LOG_WARN(mux_module, "Direct Link Pickles election send failed: %s", strerror(errno));
    election_next_tx = now + LINK_ELECTION_SEND_MS;
}

static void election_tick(const uint32_t now) {
    election_configure();
    if (election_fd < 0) return;

    election_receive(now);
    if (!direct_paired && SDL_TICKS_PASSED(now, election_opened_at + LINK_ELECTION_HOST_DELAY_MS))
        election_apply_role(link_mode_host);
    election_send(now);
}
