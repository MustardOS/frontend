...
typedef int (*mdns_record_callback_fn)(
    int sock, const struct sockaddr *from, size_t addrlen, mdns_entry_type_t entry, uint16_t query_id, uint16_t rtype,
    uint16_t rclass, uint32_t ttl, const void *data, size_t size, size_t name_offset, size_t name_length,
...
mdns_socket_listen(int sock, void *buffer, size_t capacity, mdns_record_callback_fn callback, void *user_data);
...
static inline size_t
mdns_discovery_recv(int sock, void *buffer, size_t capacity, mdns_record_callback_fn callback, void *user_data);
...
static inline int mdns_query_send(
    int sock, mdns_record_type_t type, const char *name, size_t length, void *buffer, size_t capacity, uint16_t query_id
);
...
static inline int mdns_multiquery_send(
    int sock, const mdns_query_t *query, size_t count, void *buffer, size_t capacity, uint16_t query_id
);
...
static inline size_t mdns_query_recv(
    int sock, void *buffer, size_t capacity, mdns_record_callback_fn callback, void *user_data, int query_id
);
...
static inline int mdns_query_answer_unicast(
    int sock, const void *address, size_t address_size, void *buffer, size_t capacity, uint16_t query_id,
    mdns_record_type_t record_type, const char *name, size_t name_length, mdns_record_t answer,
    const mdns_record_t *authority, size_t authority_count, const mdns_record_t *additional, size_t additional_count
);
...
static inline int mdns_query_answer_multicast(
    int sock, void *buffer, size_t capacity, mdns_record_t answer, const mdns_record_t *authority,
    size_t authority_count, const mdns_record_t *additional, size_t additional_count
);
...
mdns_record_parse_a(const void *buffer, size_t size, size_t offset, size_t length, struct sockaddr_in *addr);

//! Parse an AAAA record, returns the IPv6 address in the record
static inline struct sockaddr_in6 *
mdns_record_parse_aaaa(const void *buffer, size_t size, size_t offset, size_t length, struct sockaddr_in6 *addr);
...
static inline size_t mdns_record_parse_txt(
    const void *buffer, size_t size, size_t offset, size_t length, mdns_record_txt_t *records, size_t capacity
);
...
static inline int mdns_string_equal(
    const void *buffer_lhs, size_t size_lhs, size_t *ofs_lhs, const void *buffer_rhs, size_t size_rhs, size_t *ofs_rhs
);

static inline void *mdns_string_make(
    void *buffer, size_t capacity, void *data, const char *name, size_t length, mdns_string_table_t *string_table
);

static inline size_t mdns_string_table_find(
    mdns_string_table_t *string_table, const void *buffer, size_t capacity, const char *str, size_t first_length,
    size_t total_length
);
...
static inline int mdns_socket_setup_ipv4(int sock, const struct sockaddr_in *saddr) {
    ...
    if (bind(sock, (struct sockaddr *) &sock_addr, sizeof(struct sockaddr_in))) return -1;
...
static inline int mdns_socket_setup_ipv6(int sock, const struct sockaddr_in6 *saddr) {
    ...
    if (bind(sock, (struct sockaddr *) &sock_addr, sizeof(struct sockaddr_in6))) return -1;
...
static inline int mdns_string_skip(const void *buffer, size_t size, size_t *offset) {
    ...
    do {
        ...
        if ((substr.offset == MDNS_INVALID_POS) || (counter++ > MDNS_MAX_SUBSTRINGS)) return 0;
        if (substr.ref) {
            *offset = cur + 2;
            return 1;
        }
...
static inline int mdns_string_equal(
    ...
    do {
        ...
        if (strncasecmp(
                (const char *) MDNS_POINTER_OFFSET_CONST(buffer_rhs, rhs_substr.offset),
...
mdns_string_extract(const void *buffer, size_t size, size_t *offset, char *str, size_t capacity) {
    ...
    do {
        substr = mdns_get_next_substring(buffer, size, cur);
        if ((substr.offset == MDNS_INVALID_POS) || (counter++ > MDNS_MAX_SUBSTRINGS)) return result;
...
static inline size_t mdns_string_table_find(
    ...
    for (size_t istr = 0; istr < string_table->count; ++istr) {
        ...
        if (memcmp(str, MDNS_POINTER_OFFSET(buffer, sub_string.offset), sub_string.length)) continue;
        ...
        while (offset < total_length) {
            ...
            if (!sub_string.length || (sub_string.length != current_length)) break;
            if (memcmp(str + offset, MDNS_POINTER_OFFSET(buffer, sub_string.offset), sub_string.length)) break;
...
static inline void mdns_string_table_add(mdns_string_table_t *string_table, size_t offset) {
    ...
    if (++string_table->count > table_capacity) string_table->count = table_capacity;
...
static inline void *mdns_string_make(
    ...
    while (last_pos < length) {
        ...
        if (ref_offset != MDNS_INVALID_POS) return mdns_string_make_ref(data, remain, ref_offset);
...
static inline size_t mdns_records_parse(
    int sock, const struct sockaddr *from, size_t addrlen, const void *buffer, size_t size, size_t *offset,
    mdns_entry_type_t type, uint16_t query_id, size_t records, mdns_record_callback_fn callback, void *user_data
) {
    size_t parsed = 0;
    for (size_t i = 0; i < records; ++i) {
        ...
        *offset += 10;
        ...
        if (length <= (size - (*offset))) {
            ...
            if (callback
                && callback(
                    sock, from, addrlen, type, query_id, rtype, rclass, ttl, buffer, size, name_offset, name_length,
...
mdns_unicast_send(int sock, const void *address, size_t address_size, const void *buffer, size_t size) {
    if (sendto(
            sock, (const char *) buffer, (mdns_size_t) size, 0, (const struct sockaddr *) address,
...
mdns_discovery_recv(int sock, void *buffer, size_t capacity, mdns_record_callback_fn callback, void *user_data) {
    ...
    for (i = 0; i < questions; ++i) {
        ...
        if (!mdns_string_equal(
                buffer, data_size, &offset, mdns_services_query, sizeof(mdns_services_query), &verify_offset
            ))
            return 0;
    ...
    for (i = 0; i < answer_rrs; ++i) {
        ...
        if (is_answer) {
            ...
            if (callback
                && callback(
                    sock, saddr, addrlen, MDNS_ENTRYTYPE_ANSWER, query_id, rtype, rclass, ttl, buffer, data_size,
    ...
    records = mdns_records_parse(
        sock, saddr, addrlen, buffer, data_size, &offset, MDNS_ENTRYTYPE_AUTHORITY, query_id, authority_rrs, callback,
        user_data
    );
    ...
    records = mdns_records_parse(
        sock, saddr, addrlen, buffer, data_size, &offset, MDNS_ENTRYTYPE_ADDITIONAL, query_id, additional_rrs, callback,
        user_data
    );
    total_records += records;
...
mdns_socket_listen(int sock, void *buffer, size_t capacity, mdns_record_callback_fn callback, void *user_data) {
    ...
    for (int iquestion = 0; iquestion < questions; ++iquestion) {
        ...
        if (callback
            && callback(
                sock, saddr, addrlen, MDNS_ENTRYTYPE_QUESTION, query_id, rtype, rclass, 0, buffer, data_size,
    ...
    size_t offset = MDNS_POINTER_DIFF(data, buffer);
    records = mdns_records_parse(
        sock, saddr, addrlen, buffer, data_size, &offset, MDNS_ENTRYTYPE_ANSWER, query_id, answer_rrs, callback,
        user_data
    );
    total_records += records;
...
static inline int mdns_multiquery_send(
    ...
    if (!count || (capacity < (sizeof(struct mdns_header_t) + (6 * count)))) return -1;
...
static inline size_t mdns_query_recv(
    int sock, void *buffer, size_t capacity, mdns_record_callback_fn callback, void *user_data, int only_query_id
) {
    struct sockaddr_in6 addr;
    struct sockaddr *saddr = (struct sockaddr *) &addr;
    socklen_t addrlen = sizeof(addr);
...
static inline void *mdns_answer_add_question_unicast(
    void *buffer, size_t capacity, void *data, mdns_record_type_t record_type, const char *name, size_t name_length,
...
static inline void *mdns_answer_add_record_header(
    void *buffer, size_t capacity, void *data, mdns_record_t record, mdns_string_table_t *string_table
) {
    ...
    switch (record.type) {
        case MDNS_RECORDTYPE_PTR:
            data = mdns_string_make(
                buffer, capacity, data, record.data.ptr.name.str, record.data.ptr.name.length, string_table
            );
            break;
        ...
        case MDNS_RECORDTYPE_SRV:
            ...
            data = mdns_string_make(
                buffer, capacity, data, record.data.srv.name.str, record.data.srv.name.length, string_table
            );
            break;
...
static inline void mdns_record_update_rclass_ttl(mdns_record_t *record, uint16_t rclass, uint32_t ttl) {
    ...
    if (record->type == MDNS_RECORDTYPE_PTR) record->rclass &= ~(uint16_t) MDNS_CACHE_FLUSH;
}

static inline void *mdns_answer_add_txt_record(
    void *buffer, size_t capacity, void *data, const mdns_record_t *records, size_t record_count, uint16_t rclass,
    ...
    // Fill record length
    if (record_data) mdns_htons(record_length, (uint16_t) MDNS_POINTER_DIFF(data, record_data));
...
static inline int mdns_answer_multicast_rclass_ttl(
    int sock, void *buffer, size_t capacity, mdns_record_t answer, const mdns_record_t *authority,
    size_t authority_count, const mdns_record_t *additional, size_t additional_count, uint16_t rclass, uint32_t ttl
) {
    if (capacity < (sizeof(struct mdns_header_t) + 32 + 4)) return -1;
...
static inline int mdns_query_answer_multicast(
    ...
    return mdns_answer_multicast_rclass_ttl(
        sock, buffer, capacity, answer, authority, authority_count, additional, additional_count, MDNS_CLASS_IN, 60
    );
}
...
static inline int mdns_goodbye_multicast(
    ...
    return mdns_answer_multicast_rclass_ttl(
        sock, buffer, capacity, answer, authority, authority_count, additional, additional_count, MDNS_CLASS_IN, 0
    );
}
...
#endif
    if ((size >= offset + length) && (length == 16)) memcpy(&addr->sin6_addr, MDNS_POINTER_OFFSET(buffer, offset), 16);
    return addr;
}
