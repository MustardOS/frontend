...
static lv_coord_t get_row_height(
    lv_obj_t *obj, uint16_t row_id, const lv_font_t *font, lv_coord_t letter_space, lv_coord_t line_space,
...
static inline int is_cell_empty(void *cell) {
    return cell == NULL;
}
...
void lv_table_set_cell_value_fmt(lv_obj_t *obj, uint16_t row, uint16_t col, const char *fmt, ...) {
    ...
    table->cell_data[cell] =
        lv_mem_realloc(table->cell_data[cell], sizeof(lv_table_cell_t) + len + 1); /*+1: trailing '\0; */
    LV_ASSERT_MALLOC(table->cell_data[cell]);
    if (table->cell_data[cell] == NULL) {
        va_end(ap2);
        return;
...
void lv_table_set_col_cnt(lv_obj_t *obj, uint16_t col_cnt) {
    ...
    for (row = 0; row < table->row_cnt; row++) {
        ...
        lv_memcpy_small(
            &new_cell_data[new_col_start], &table->cell_data[old_col_start], sizeof(new_cell_data[0]) * min_col_cnt
        );
...
int lv_table_has_cell_ctrl(lv_obj_t *obj, uint16_t row, uint16_t col, lv_table_cell_ctrl_t ctrl) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
...
static void draw_main(lv_event_t *e) {
    ...
    int rtl = lv_obj_get_style_base_dir(obj, LV_PART_MAIN) == LV_BASE_DIR_RTL;
    ...
    for (row = 0; row < table->row_cnt; row++) {
        ...
        for (col = 0; col < table->col_cnt; col++) {
            ...
            if (table->cell_data[cell]) {
                ...
                int crop = ctrl & LV_TABLE_CELL_CTRL_TEXT_CROP ? 1 : 0;
                if (crop) txt_flags = LV_TEXT_FLAG_EXPAND;

                lv_txt_get_size(
                    &txt_size, table->cell_data[cell]->txt, label_dsc_def.font, label_dsc_act.letter_space,
...
static void refr_size_form_row(lv_obj_t *obj, uint32_t start_row) {
    ...
    for (i = start_row; i < table->row_cnt; i++) {
        lv_coord_t calculated_height = get_row_height(
            obj, i, font, letter_space, line_space, cell_pad_left, cell_pad_right, cell_pad_top, cell_pad_bottom
        );
...
static void refr_cell_size(lv_obj_t *obj, uint32_t row, uint32_t col) {
    ...
    lv_table_t *table = (lv_table_t *) obj;
    lv_coord_t calculated_height = get_row_height(
        obj, row, font, letter_space, line_space, cell_pad_left, cell_pad_right, cell_pad_top, cell_pad_bottom
    );
...
static lv_coord_t get_row_height(
    ...
    for (cell = row_start, col = 0; cell < row_start + table->col_cnt; cell++, col++) {
        ...
        /*Else we have to calculate the height of the cell text*/
            ...
            lv_txt_get_size(
                &txt_size, table->cell_data[cell]->txt, font, letter_space, line_space, txt_w, LV_TEXT_FLAG_NONE
            );
