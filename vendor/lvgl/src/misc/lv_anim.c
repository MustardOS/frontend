...
static int anim_list_changed;
static int anim_run_round;
...
uint32_t lv_anim_get_playtime(lv_anim_t *a) {
    ...
    if (a->repeat_cnt == LV_ANIM_REPEAT_INFINITE) return playtime;

    playtime = a->time - a->act_time;
    if (a->playback_now == 0) playtime += a->playback_delay + a->playback_time;

    if (a->repeat_cnt <= 1) return playtime;

    playtime += (a->repeat_delay + a->time + a->playback_delay + a->playback_time) * (a->repeat_cnt - 1);

    return playtime;
}

int lv_anim_del(void *var, lv_anim_exec_xcb_t exec_cb) {
    lv_anim_t *a;
    lv_anim_t *a_next;
    int del = 0;
...
static void anim_timer(lv_timer_t *param) {
    ...
    /*Flip the run round*/
    anim_run_round = anim_run_round ? 0 : 1;
    ...
    while (a != NULL) {
        ...
        if (a->run_round != anim_run_round) {
            a->run_round =
                anim_run_round; /*The list readying might be reset so need to know which anim has run already*/
