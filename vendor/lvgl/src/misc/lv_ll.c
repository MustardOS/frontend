 ...
 * @param head 1: be the head in the new list
 *             0 be the tail in the new list
 */
void _lv_ll_chg_list(lv_ll_t *ll_ori_p, lv_ll_t *ll_new_p, void *node, int head) {
 ...
 * @param ll_p pointer to a linked list
 * @return 1: the linked list is empty; 0: not empty
 */
int _lv_ll_is_empty(lv_ll_t *ll_p) {
    if (ll_p == NULL) return 1;

    if (ll_p->head == NULL && ll_p->tail == NULL) return 1;
