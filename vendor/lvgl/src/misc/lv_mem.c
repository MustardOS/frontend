...
void *lv_mem_alloc(size_t size) {
    ...
    if (alloc == NULL) {
        ...
        lv_mem_monitor(&mon);
        LV_LOG_INFO(
            "used: %6d (%3d %%), frag: %3d %%, biggest free: %6d", (int) (mon.total_size - mon.free_size), mon.used_pct,
...
void *lv_mem_buf_get(uint32_t size) {
    ...
    if (i_guess >= 0) {
        ...
        MEM_TRACE(
            "returning already allocated buffer (buffer id: %d, address: %p)", i_guess,
    ...
    for (uint8_t i = 0; i < LV_MEM_BUF_MAX_NUM; i++) {
        if (LV_GC_ROOT(lv_mem_buf[i]).used == 0) {
            ...
            LV_ASSERT_MSG(
                buf != NULL, "Out of memory, can't allocate a new buffer (increase your LV_MEM_SIZE/heap size)"
            );
    ...
    LV_ASSERT_MSG(0, "No more buffers. Increase LV_MEM_BUF_MAX_NUM.");
    return NULL;
}
...
static void lv_mem_walker(void *ptr, size_t size, int used, void *user) {
    ...
    if (used) {
        ...
        if (size > mon_p->free_biggest_size) mon_p->free_biggest_size = size;
    }
}
#endif
