...
static int lv_point_within_circle(const lv_area_t *area, const lv_point_t *p);
...
int _lv_area_is_in(const lv_area_t *ain_p, const lv_area_t *aholder_p, lv_coord_t radius) {
    ...
    p.x = ain_p->x1;
    p.y = ain_p->y1;
    if (_lv_area_is_point_on(aholder_p, &p, radius) == 0) return 0;
...
int _lv_area_is_out(const lv_area_t *aout_p, const lv_area_t *aholder_p, lv_coord_t radius) {
    ...
    p.x = aout_p->x1;
    p.y = aout_p->y1;
    if (_lv_area_is_point_on(aholder_p, &p, radius)) return 0;
