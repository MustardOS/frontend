...
int lv_fs_is_ready(char letter) {
    lv_fs_drv_t *drv = lv_fs_get_drv(letter);

    if (drv == NULL) return 0; /*An unknown driver in not ready*/

    if (drv->ready_cb == NULL) return 1; /*Assume the driver is always ready if no handler provided*/
...
static lv_fs_res_t lv_fs_read_cached(lv_fs_file_t *file_p, char *buf, uint32_t btr, uint32_t *br) {
    ...
    if (start <= file_position && file_position < end) {
        ...
        uint32_t buffer_remaining_length =
            LV_MIN((uint32_t) buffer_size - buffer_offset, (uint32_t) end - file_position);
            ...
            *br = btr;
            ...
            if (btr > buffer_size) {
                ...
                res = file_p->drv->read_cb(
                    file_p->drv, file_p->file_d, (void *) (buf + buffer_remaining_length),
                ...
                res = file_p->drv->read_cb(
                    file_p->drv, file_p->file_d, (void *) buffer, buffer_size, &bytes_read_to_buffer
                );
        ...
        if (btr > buffer_size) {
            ...
            uint32_t bytes_read_to_buffer = 0;
            res =
                file_p->drv->read_cb(file_p->drv, file_p->file_d, (void *) buffer, buffer_size, &bytes_read_to_buffer);
...
lv_fs_res_t lv_fs_seek(lv_fs_file_t *file_p, uint32_t pos, lv_fs_whence_t whence) {
    ...
    if (file_p->drv->cache_size) {
        switch (whence) {
            ...
            case LV_FS_SEEK_END: {
                /*Because we don't know the file size, we do a little trick: do a FS seek, then get new file position
                 * from FS*/
