...
void lv_draw_sdl_draw_letter(
    ...
    int g_ret = lv_font_get_glyph_dsc(font_p, &g, letter, '\0');
    if (g_ret == 0) {
    ...
    int has_composite = lv_draw_sdl_composite_begin(
        ctx, &letter_area, clip_area, NULL, dsc->blend_mode, &t_letter, &t_clip, &apply_area
    );
