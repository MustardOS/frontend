...
static void
dump_masks(SDL_Texture *texture, const lv_area_t *coords, const int16_t *ids, int16_t ids_count, const int16_t *caps);

static void
get_cap_area(int16_t angle, lv_coord_t thickness, uint16_t radius, const lv_point_t *center, lv_area_t *out);
...
void lv_draw_sdl_draw_arc(
    ...
    lv_draw_mask_radius_init(&mask_out_param, &area_out, LV_RADIUS_CIRCLE, 0);
    mask_ids[0] = lv_draw_mask_add(&mask_out_param, NULL);

    lv_draw_mask_radius_param_t mask_in_param;
    if (lv_area_get_width(&area_in) > 0 && lv_area_get_height(&area_in) > 0) {
        lv_draw_mask_radius_init(&mask_in_param, &area_in, LV_RADIUS_CIRCLE, 1);
    ...
    if (mask_ids_count == 3 && dsc->rounded) {
        ...
        lv_draw_mask_radius_init(&cap_start_param, &start_area, dsc->width / 2, 0);
        cap_ids[0] = lv_draw_mask_add(&cap_start_param, NULL);
        lv_draw_mask_radius_init(&cap_end_param, &end_area, dsc->width / 2, 0);
        cap_ids[1] = lv_draw_mask_add(&cap_end_param, NULL);
    }

    lv_coord_t w = lv_area_get_width(&draw_area), h = lv_area_get_height(&draw_area);
    int texture_cached = 0;
    SDL_Texture *texture =
        lv_draw_sdl_composite_texture_obtain(ctx, LV_DRAW_SDL_COMPOSITE_TEXTURE_ID_STREAM1, w, h, &texture_cached);
