...
static int check_mask_simple_radius(const lv_area_t *coords, lv_coord_t *radius);

static void draw_img_simple(
...
static void draw_img_rounded(
    lv_draw_sdl_ctx_t *ctx, SDL_Texture *texture, const lv_draw_sdl_img_header_t *header,
    const lv_draw_img_dsc_t *draw_dsc, const lv_area_t *coords, const lv_area_t *clip, lv_coord_t radius
);

static SDL_Texture *img_rounded_frag_obtain(
    lv_draw_sdl_ctx_t *ctx, SDL_Texture *texture, const lv_draw_sdl_img_header_t *header, int w, int h,
    lv_coord_t radius, int *in_cache
);

static lv_draw_img_rounded_key_t
rounded_key_create(const SDL_Texture *texture, lv_coord_t w, lv_coord_t h, lv_coord_t radius);

static void calc_draw_part(
    SDL_Texture *texture, const lv_draw_sdl_img_header_t *header, const lv_area_t *coords, const lv_area_t *clip,
...
lv_res_t lv_draw_sdl_img_core(
    ...
    SDL_Texture *texture =
        lv_draw_sdl_texture_cache_get_with_userdata(ctx, key, key_size, &texture_found, (void **) &header);
    int texture_in_cache = 0;
    if (!texture_found) {
        lv_draw_sdl_img_load_texture(ctx, key, key_size, src, draw_dsc->frame_id, &texture, &header, &texture_in_cache);
    } else {
        texture_in_cache = 1;
    }
    SDL_free(key);
    ...
    _lv_img_buf_get_transformed_area(
        &zoomed_cords, lv_area_get_width(coords), lv_area_get_height(coords), 0, draw_dsc->zoom, &draw_dsc->pivot
    );
    ...
    if (!check_mask_simple_radius(&t_coords, &radius)) {
        has_composite = lv_draw_sdl_composite_begin(
            ctx, &zoomed_cords, clip, NULL, draw_dsc->blend_mode, &t_coords, &t_clip, &apply_area
        );
    }
...
int lv_draw_sdl_img_load_texture(
    lv_draw_sdl_ctx_t *ctx, lv_draw_sdl_cache_key_head_img_t *key, size_t key_size, const void *src, int32_t frame_id,
    SDL_Texture **texture, lv_draw_sdl_img_header_t **header, int *texture_in_cache
) {
...
static SDL_Texture *upload_img_texture(SDL_Renderer *renderer, lv_img_decoder_dsc_t *dsc) {
    ...
    int chroma_keyed = dsc->header.cf == (uint32_t) LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED;
    ...
    SDL_Surface *surface =
        SDL_CreateRGBSurfaceFrom(data, w, h, LV_COLOR_DEPTH, w * LV_COLOR_DEPTH / 8, rmask, gmask, bmask, amask);
 ...
 * @return 1 if the only mask is a radius mask
 */
static int check_mask_simple_radius(const lv_area_t *coords, lv_coord_t *radius) {
    if (lv_draw_mask_get_cnt() != 1) return 0;
    for (uint8_t i = 0; i < _LV_MASK_MAX_NUM; i++) {
        ...
        if (param->type == LV_DRAW_MASK_TYPE_RADIUS) {
            ...
            if (rparam->cfg.outer) return 0;
            if (!_lv_area_is_equal(&rparam->cfg.rect, coords)) return 0;
...
static void draw_img_simple(
    ...
    if (draw_dsc->recolor_opa > LV_OPA_TRANSP && src_rect.w > 0 && src_rect.h > 0) {
        /*
         * Two-pass recolour: SDL_SetTextureColorMod is multiplicative so it cannot
         * recolour dark/black pixels. Instead we fill a temp render target with the
         * target colour, then stamp the source texture's alpha channel onto it using a
         * custom blend mode (keep dst RGB, replace dst A with src A).
         */
        lv_color_t recolor = lv_color_mix(draw_dsc->recolor, lv_color_white(), draw_dsc->recolor_opa);
        SDL_Rect recolor_rect = {0, 0, src_rect.w, src_rect.h};

        int temp_in_cache = 0;
        SDL_Texture *temp = lv_draw_sdl_composite_texture_obtain(
            ctx, LV_DRAW_SDL_COMPOSITE_TEXTURE_ID_TARGET1, src_rect.w, src_rect.h, &temp_in_cache
        );
        if (temp != NULL) {
            SDL_Texture *old_target = SDL_GetRenderTarget(ctx->renderer);
            SDL_RenderSetClipRect(ctx->renderer, NULL);
            SDL_SetRenderTarget(ctx->renderer, temp);

            SDL_SetRenderDrawBlendMode(ctx->renderer, SDL_BLENDMODE_NONE);
            SDL_SetRenderDrawColor(ctx->renderer, recolor.ch.red, recolor.ch.green, recolor.ch.blue, 0);
            SDL_RenderFillRect(ctx->renderer, &recolor_rect);

            SDL_BlendMode alpha_stamp = SDL_ComposeCustomBlendMode(
                SDL_BLENDFACTOR_ZERO, SDL_BLENDFACTOR_ONE, SDL_BLENDOPERATION_ADD, SDL_BLENDFACTOR_ONE,
                SDL_BLENDFACTOR_ZERO, SDL_BLENDOPERATION_ADD
            );
            SDL_SetTextureBlendMode(texture, alpha_stamp);
            SDL_SetTextureColorMod(texture, 0xFF, 0xFF, 0xFF);
            SDL_SetTextureAlphaMod(texture, 0xFF);
            SDL_RenderCopy(ctx->renderer, texture, &src_rect, &recolor_rect);
            SDL_SetTextureBlendMode(texture, SDL_BLENDMODE_BLEND);
            ...
            SDL_SetTextureColorMod(temp, 0xFF, 0xFF, 0xFF);
            SDL_SetTextureAlphaMod(temp, draw_dsc->opa);
            ...
            SDL_RenderCopyEx(ctx->renderer, temp, &recolor_rect, &dst_rect, draw_dsc->angle, &pivot, SDL_FLIP_NONE);
...
static void draw_img_rounded(
    ...
    lv_draw_sdl_rect_bg_frag_draw_corners(ctx, frag, real_radius, coords, clip, 1);
...
static SDL_Texture *img_rounded_frag_obtain(
    ...
    if (img_frag == NULL) {
        ...
        img_frag = SDL_CreateTexture(
            ctx->renderer, LV_DRAW_SDL_TEXTURE_FORMAT, SDL_TEXTUREACCESS_TARGET, full_frag_size, full_frag_size
        );
        SDL_assert(img_frag);
        ...
        lv_draw_sdl_rect_bg_frag_draw_corners(ctx, mask_frag, radius, &frag_coords, NULL, 0);

        SDL_SetTextureAlphaMod(texture, 0xFF);
        SDL_SetTextureColorMod(texture, 0xFF, 0xFF, 0xFF);
#if LV_GPU_SDL_CUSTOM_BLEND_MODE
        SDL_BlendMode blend_mode = SDL_ComposeCustomBlendMode(
            SDL_BLENDFACTOR_ONE, SDL_BLENDFACTOR_ZERO, SDL_BLENDOPERATION_ADD, SDL_BLENDFACTOR_DST_ALPHA,
            SDL_BLENDFACTOR_ZERO, SDL_BLENDOPERATION_ADD
        );
