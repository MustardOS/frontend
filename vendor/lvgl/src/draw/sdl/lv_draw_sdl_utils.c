...
void lv_area_zoom_to_sdl_rect(const lv_area_t *in, SDL_Rect *out, uint16_t zoom, const lv_point_t *pivot) {
    ...
    /* Compute destination rect directly rather than via _lv_img_buf_get_transformed_area.
     * That function adds ±2px anti-aliasing padding which makes the x/y scale ratios
     * non-uniform for objects where height << width (e.g. list items), causing visible
     * aspect distortion. SDL handles pixel precision via bilinear filtering, not padding. */
    lv_coord_t w = lv_area_get_width(in);
    lv_coord_t h = lv_area_get_height(in);
    out->x = in->x1 + (int) ((int32_t) pivot->x * (256 - (int32_t) zoom) / 256);
    out->y = in->y1 + (int) ((int32_t) pivot->y * (256 - (int32_t) zoom) / 256);
    out->w = (int) ((int32_t) w * zoom / 256);
    out->h = (int) ((int32_t) h * zoom / 256);
}
...
SDL_Texture *lv_sdl_create_opa_texture(
    SDL_Renderer *renderer, lv_opa_t *pixels, lv_coord_t width, lv_coord_t height, lv_coord_t stride
) {
