...
static int pinyin_k9_is_valid_py(lv_obj_t *obj, char *py_str);
...
void lv_ime_pinyin_set_mode(lv_obj_t *obj, lv_ime_pinyin_mode_t mode) {
    ...
    if (pinyin_ime->mode == LV_IME_PINYIN_MODE_K9) {
        ...
        lv_keyboard_set_map(
            pinyin_ime->kb, LV_KEYBOARD_MODE_USER_1, (const char *) lv_btnm_def_pinyin_k9_map,
...
static void lv_ime_pinyin_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj) {
    ...
    lv_btnmatrix_set_one_checked(pinyin_ime->cand_panel, 1);
    ...
    if (code == LV_EVENT_VALUE_CHANGED) {
        ...
        if (strcmp(txt, "Enter") == 0 || strcmp(txt, LV_SYMBOL_NEW_LINE) == 0) {
            ...
            if (pinyin_ime->ta_count > 0) {
                if (pinyin_ime->mode == LV_IME_PINYIN_MODE_K26) pinyin_ime->input_char[pinyin_ime->ta_count - 1] = '\0';
#if LV_IME_PINYIN_USE_K9_MODE
                else
...
static int pinyin_k9_is_valid_py(lv_obj_t *obj, char *py_str) {
    ...
    if (*py_str == '\0') return 0;
    if (*py_str == 'i') return 0;
    if (*py_str == 'u') return 0;
    if (*py_str == 'v') return 0;
...
static void pinyin_k9_fill_cand(lv_obj_t *obj) {
    ...
    while (ll_index) {
        if ((index >= LV_IME_PINYIN_K9_CAND_TEXT_NUM) || (index >= pinyin_ime->k9_legal_py_count)) break;
...
static void pinyin_k9_cand_page_proc(lv_obj_t *obj, uint16_t dir) {
    ...
    if ((ll_len > LV_IME_PINYIN_K9_CAND_TEXT_NUM) && (pinyin_ime->k9_legal_py_count > LV_IME_PINYIN_K9_CAND_TEXT_NUM)) {
        ...
        if (dir == 1) {
            count = 0;
            while (ll_index) {
                if (count >= (LV_IME_PINYIN_K9_CAND_TEXT_NUM - 1)) break;
        ...
        // previous page
            ...
            if (pinyin_ime->k9_py_ll_pos > LV_IME_PINYIN_K9_CAND_TEXT_NUM) pinyin_ime->k9_py_ll_pos -= 1;
        }
