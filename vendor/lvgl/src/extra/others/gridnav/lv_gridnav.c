...
static int obj_is_focuable(lv_obj_t *obj);
...
static void gridnav_event_cb(lv_event_t *e) {
    ...
    if (code == LV_EVENT_KEY) {
        ...
        if (dsc->focused_obj) {
            ...
            lv_obj_clear_state(
                dsc->focused_obj, LV_STATE_PRESSED
            ); /*Be sure the focuses obj is not stuck in pressed state*/
        ...
        lv_gridnav_remove(obj);
               ...
               || code == LV_EVENT_LONG_PRESSED || code == LV_EVENT_LONG_PRESSED_REPEAT || code == LV_EVENT_CLICKED
...
static lv_obj_t *find_chid(lv_obj_t *obj, lv_obj_t *start_child, find_mode_t mode) {
    ...
    for (i = 0; i < child_cnt; i++) {
        ...
        if (obj_is_focuable(child) == 0) continue;
