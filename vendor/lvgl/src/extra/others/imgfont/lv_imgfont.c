...
static int
imgfont_get_glyph_dsc(const lv_font_t *font, lv_font_glyph_dsc_t *dsc_out, uint32_t unicode, uint32_t unicode_next);
...
lv_font_t *lv_imgfont_create(uint16_t height, lv_get_imgfont_path_cb_t path_cb) {
    ...
    imgfont_dsc_t *dsc = lv_mem_alloc(size);
