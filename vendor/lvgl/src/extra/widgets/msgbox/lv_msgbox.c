...
const lv_obj_class_t lv_msgbox_backdrop_class = {
    .base_class = &lv_obj_class, .width_def = LV_PCT(100), .height_def = LV_PCT(100), .instance_size = sizeof(lv_obj_t)
};
...
lv_obj_t *
lv_msgbox_create(lv_obj_t *parent, const char *title, const char *txt, const char *btn_txts[], int add_close_btn) {
    LV_LOG_INFO("begin");
    int auto_parent = 0;
    if (parent == NULL) {
        auto_parent = 1;
    ...
    int has_title = title && strlen(title) > 0;
