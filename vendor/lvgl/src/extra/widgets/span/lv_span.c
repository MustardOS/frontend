...
lv_coord_t lv_spangroup_get_expand_height(lv_obj_t *obj, lv_coord_t width) {
    ...
    while (cur_span) {
        ...
        while (1) {
            ...
            int isfill = lv_txt_get_snippet(
                &cur_txt[cur_txt_ofs], snippet.font, snippet.letter_space, max_w, txt_flag, &use_width, &next_ofs
            );
...
/**
 * @return 1 for txt fill the max_width.
 */
static int lv_txt_get_snippet(
    const char *txt, const lv_font_t *font, lv_coord_t letter_space, lv_coord_t max_width, lv_text_flag_t flag,
    lv_coord_t *use_width, uint32_t *end_ofs
            ...
            if (txt_pos.y + max_line_h + next_line_h - line_space > coords.y2 + 1) { /* for overflow if is end line. */
                if (last_snippet->txt[last_snippet->bytes] != '\0') {
                    ...
                    last_snippet->txt_w = lv_txt_get_width(
                        last_snippet->txt, last_snippet->bytes, last_snippet->font, last_snippet->letter_space, txt_flag
                    );
                }
                ellipsis_valid = spans->overflow == LV_SPAN_OVERFLOW_ELLIPSIS ? 1 : 0;
                is_end_line = 1;
            }
        }
