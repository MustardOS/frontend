...
void lv_meter_set_scale_ticks(
    lv_obj_t *obj, lv_meter_scale_t *scale, uint16_t cnt, uint16_t width, uint16_t len, lv_color_t color
) {
    scale->tick_cnt = cnt;
...
void lv_meter_set_scale_major_ticks(
    lv_obj_t *obj, lv_meter_scale_t *scale, uint16_t nth, uint16_t width, uint16_t len, lv_color_t color,
    int16_t label_gap
) {
...
void lv_meter_set_scale_range(
    lv_obj_t *obj, lv_meter_scale_t *scale, int32_t min, int32_t max, uint32_t angle_range, uint32_t rotation
) {
    scale->min = min;
    scale->max = max;
...
lv_meter_add_needle_line(lv_obj_t *obj, lv_meter_scale_t *scale, uint16_t width, lv_color_t color, int16_t r_mod) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_meter_t *meter = (lv_meter_t *) obj;
...
lv_meter_indicator_t *lv_meter_add_needle_img(
    lv_obj_t *obj, lv_meter_scale_t *scale, const void *src, lv_coord_t pivot_x, lv_coord_t pivot_y
) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_meter_t *meter = (lv_meter_t *) obj;
    lv_meter_indicator_t *indic = _lv_ll_ins_head(&meter->indicator_ll);
...
lv_meter_indicator_t *
lv_meter_add_arc(lv_obj_t *obj, lv_meter_scale_t *scale, uint16_t width, lv_color_t color, int16_t r_mod) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_meter_t *meter = (lv_meter_t *) obj;
    lv_meter_indicator_t *indic = _lv_ll_ins_head(&meter->indicator_ll);
    LV_ASSERT_MALLOC(indic);
...
lv_meter_indicator_t *lv_meter_add_scale_lines(
    lv_obj_t *obj, lv_meter_scale_t *scale, lv_color_t color_start, lv_color_t color_end, int local, int16_t width_mod
) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_meter_t *meter = (lv_meter_t *) obj;
    lv_meter_indicator_t *indic = _lv_ll_ins_head(&meter->indicator_ll);
...
static void draw_arcs(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, const lv_area_t *scale_area) {
    ...
    _LV_LL_READ_BACK(&meter->indicator_ll, indic) {
        ...
        int32_t start_angle =
            lv_map(indic->start_value, scale->min, scale->max, scale->rotation, scale->rotation + scale->angle_range);
        int32_t end_angle =
            lv_map(indic->end_value, scale->min, scale->max, scale->rotation, scale->rotation + scale->angle_range);
...
static void draw_ticks_and_labels(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, const lv_area_t *scale_area) {
    ...
    _LV_LL_READ_BACK(&meter->scale_ll, scale) {
        ...
        lv_draw_mask_radius_init(&inner_minor_mask, &area_inner_minor, LV_RADIUS_CIRCLE, 1);
        ...
        lv_draw_mask_radius_init(&inner_major_mask, &area_inner_major, LV_RADIUS_CIRCLE, 1);
        ...
        lv_draw_mask_radius_init(&outer_mask, &area_outer, LV_RADIUS_CIRCLE, 0);
        ...
        for (i = 0; i < scale->tick_cnt; i++) {
                ...
                if (value_of_line >= indic->start_value && value_of_line <= indic->end_value) {
                    ...
                    if (indic->type_data.scale_lines.color_start.full == indic->type_data.scale_lines.color_end.full) {
                        ...
                        if (indic->type_data.scale_lines.local_grad) {
                            ratio = lv_map(
                                value_of_line, indic->start_value, indic->end_value, LV_OPA_TRANSP, LV_OPA_COVER
                            );
                        } else {
                            ratio = lv_map(value_of_line, scale->min, scale->max, LV_OPA_TRANSP, LV_OPA_COVER);
                        }
                        line_color = lv_color_mix(
                            indic->type_data.scale_lines.color_end, indic->type_data.scale_lines.color_start, ratio
                        );
                    }
                }
            }
            ...
            if (major) {
                ...
                lv_txt_get_size(
                    &label_size, part_draw_dsc.text, label_dsc_tmp.font, label_dsc_tmp.letter_space,
                    label_dsc_tmp.line_space, LV_COORD_MAX, LV_TEXT_FLAG_NONE
                );
...
static void inv_arc(lv_obj_t *obj, lv_meter_indicator_t *indic, int32_t old_value, int32_t new_value) {
    int rounded = lv_obj_get_style_arc_rounded(obj, LV_PART_ITEMS);
    ...
    int32_t start_angle =
        lv_map(old_value, scale->min, scale->max, scale->rotation, scale->angle_range + scale->rotation);
    int32_t end_angle =
        lv_map(new_value, scale->min, scale->max, scale->rotation, scale->angle_range + scale->rotation);

    lv_area_t a;
    lv_draw_arc_get_area(
        scale_center.x, scale_center.y, r_out, LV_MIN(start_angle, end_angle), LV_MAX(start_angle, end_angle),
...
static void inv_line(lv_obj_t *obj, lv_meter_indicator_t *indic, int32_t value) {
    ...
    if (indic->type == LV_METER_INDICATOR_TYPE_NEEDLE_LINE) {
        ...
        _lv_img_buf_get_transformed_area(
            &a, info.w, info.h, angle, LV_IMG_ZOOM_NONE, &indic->type_data.needle_img.pivot
        );
