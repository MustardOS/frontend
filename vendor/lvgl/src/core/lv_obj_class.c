    ...
    /*Create a normal object*/
        ...
        if (parent->spec_attr->children == NULL) {
            ...
            parent->spec_attr->children =
                lv_mem_realloc(parent->spec_attr->children, sizeof(lv_obj_t *) * parent->spec_attr->child_cnt);
...
int lv_obj_is_editable(lv_obj_t *obj) {
    ...
    if (class_p == NULL) return 0;

    return class_p->editable == LV_OBJ_CLASS_EDITABLE_TRUE ? 1 : 0;
}

int lv_obj_is_group_def(lv_obj_t *obj) {
    const lv_obj_class_t *class_p = obj->class_p;
    ...
    return class_p->group_def == LV_OBJ_CLASS_GROUP_DEF_TRUE ? 1 : 0;
}
