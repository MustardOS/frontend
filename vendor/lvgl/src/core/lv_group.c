 ...
 **********************/
static int
focus_next_core(lv_group_t *group, void *(*begin)(const lv_ll_t *), void *(*move)(const lv_ll_t *, const void *) );
...
void lv_group_focus_next(lv_group_t *group) {
    int focus_changed = focus_next_core(group, _lv_ll_get_head, _lv_ll_get_next);
    if (group->edge_cb) {
        if (!focus_changed) group->edge_cb(group, 1);
    }
}

void lv_group_focus_prev(lv_group_t *group) {
    int focus_changed = focus_next_core(group, _lv_ll_get_tail, _lv_ll_get_prev);
    if (group->edge_cb) {
        if (!focus_changed) group->edge_cb(group, 0);
    }
}
...
int lv_group_get_editing(const lv_group_t *group) {
    if (!group) return 0;
    return group->editing ? 1 : 0;
}

int lv_group_get_wrap(lv_group_t *group) {
    if (!group) return 0;
    return group->wrap ? 1 : 0;
}
