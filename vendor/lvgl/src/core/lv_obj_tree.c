...
void lv_obj_del(lv_obj_t *obj) {
    ...
    if (par == NULL) {
        ...
        if (disp->act_scr == obj) act_scr_del = 1;
    }
...
void lv_obj_set_parent(lv_obj_t *obj, lv_obj_t *parent) {
    ...
    if (old_parent->spec_attr->child_cnt) {
        old_parent->spec_attr->children =
            lv_mem_realloc(old_parent->spec_attr->children, old_parent->spec_attr->child_cnt * (sizeof(lv_obj_t *)));
    } else {
    ...
    parent->spec_attr->children =
        lv_mem_realloc(parent->spec_attr->children, parent->spec_attr->child_cnt * (sizeof(lv_obj_t *)));
