...
static void transform_point(const lv_obj_t *obj, lv_point_t *p, int inv);
...
int lv_obj_refr_size(lv_obj_t *obj) {
    LV_ASSERT_OBJ(obj, MY_CLASS);

    /*If the width or height is set by a layout do not modify them*/
    if (obj->w_layout && obj->h_layout) return 0;
    ...
    if (obj->w_layout) {
        ...
        w_is_content = w == LV_SIZE_CONTENT ? 1 : 0;
        w_is_pct = LV_COORD_IS_PCT(w) ? 1 : 0;
    ...
    if (obj->h_layout) {
        ...
        h_is_content = h == LV_SIZE_CONTENT ? 1 : 0;
        h_is_pct = LV_COORD_IS_PCT(h) ? 1 : 0;
    ...
    if (lv_obj_get_width(obj) == w && lv_obj_get_height(obj) == h) return 0;
    ...
    int on1 = _lv_area_is_in(&ori, &parent_fit_area, 0);
    ...
    int on2 = _lv_area_is_in(&obj->coords, &parent_fit_area, 0);
...
int lv_obj_is_layout_positioned(const lv_obj_t *obj) {
    if (lv_obj_has_flag_any(obj, LV_OBJ_FLAG_HIDDEN | LV_OBJ_FLAG_IGNORE_LAYOUT | LV_OBJ_FLAG_FLOATING)) return 0;
...
void lv_obj_move_to(lv_obj_t *obj, lv_coord_t x, lv_coord_t y) {
    ...
    lv_obj_move_children_by(obj, diff.x, diff.y, 0);
...
void lv_obj_move_children_by(lv_obj_t *obj, lv_coord_t x_diff, lv_coord_t y_diff, int ignore_floating) {
    uint32_t i;
    ...
    for (i = 0; i < child_cnt; i++) {
        ...
        lv_obj_move_children_by(child, x_diff, y_diff, 0);
    }
}

void lv_obj_transform_point(const lv_obj_t *obj, lv_point_t *p, int recursive, int inv) {
    if (obj) {
        lv_layer_type_t layer_type = _lv_obj_get_layer_type(obj);
        int do_tranf = layer_type == LV_LAYER_TYPE_TRANSFORM;
        if (inv) {
...
void lv_obj_get_transformed_area(const lv_obj_t *obj, lv_area_t *area, int recursive, int inv) {
    lv_point_t p[4] = {
        {area->x1, area->y1},
        {area->x1, area->y2},
        {area->x2, area->y1},
...
int lv_obj_area_is_visible(const lv_obj_t *obj, lv_area_t *area) {
    ...
    if (obj_scr != lv_disp_get_scr_act(disp) && obj_scr != lv_disp_get_scr_prev(disp)
        && obj_scr != lv_disp_get_layer_top(disp) && obj_scr != lv_disp_get_layer_sys(disp)) {
        return 0;
    }
    ...
    if (!lv_obj_has_flag_any(obj, LV_OBJ_FLAG_OVERFLOW_VISIBLE)) {
        ...
        if (!_lv_area_intersect(area, area, &obj_coords)) return 0;
    }

    lv_obj_get_transformed_area(obj, area, 1, 0);

    /*Truncate recursively to the parents*/
    lv_obj_t *par = lv_obj_get_parent(obj);
    while (par != NULL) {
        /*If the parent is hidden then the child is hidden and won't be drawn*/
        if (lv_obj_has_flag(par, LV_OBJ_FLAG_HIDDEN)) return 0;
        ...
        if (!lv_obj_has_flag_any(par, LV_OBJ_FLAG_OVERFLOW_VISIBLE)) {
            ...
            lv_obj_get_transformed_area(par, &par_area, 1, 0);
            if (!_lv_area_intersect(area, area, &par_area)) return 0;
        }
...
int lv_obj_is_visible(const lv_obj_t *obj) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
...
int lv_obj_hit_test(lv_obj_t *obj, const lv_point_t *point) {
    if (!lv_obj_has_flag(obj, LV_OBJ_FLAG_CLICKABLE)) return 0;
    if (lv_obj_has_state(obj, LV_STATE_DISABLED)) return 0;

    lv_area_t a;
    lv_obj_get_click_area(obj, &a);
    int res = _lv_area_is_point_on(&a, point, 0);
    if (res == 0) return 0;
...
static void transform_point(const lv_obj_t *obj, lv_point_t *p, int inv) {
    ...
    lv_point_t pivot = {
        .x = lv_obj_get_style_transform_pivot_x(obj, 0), .y = lv_obj_get_style_transform_pivot_y(obj, 0)
    };
