...
typedef enum {
    ...
    LV_OBJ_DRAW_PART_BORDER_POST, /**< The border if style_border_post = 1*/
 ...
 * @return      1: all flags are set; 0: not all flags are set
 */
 ...
 * @param obj   pointer to an object
 * @param f     the flag(s) to check (OR-ed values can be used)
 * @return      1: at lest one flag flag is set; 0: none of the flags are set
 */
 ...
 * @return          1: `obj` is in `state`; 0: `obj` is not in `state`
 */
 ...
 * @return          1: `class_p` is the `obj` class.
 */
 ...
 * @param obj       pointer to an object
 * @param class_p   a class to check (e.g. `lv_slider_class`)
 * @return          1: `obj` has the given class
 */
...
#define LV_ASSERT_OBJ(obj_p, obj_class)                                                                                \
    do {                                                                                                               \
        ...
        LV_ASSERT_MSG(lv_obj_has_class(obj_p, obj_class) == 1, "Incompatible object type.");                           \
        LV_ASSERT_MSG(lv_obj_is_valid(obj_p) == 1, "The object is invalid, deleted or corrupted?");                    \
    } while (0)
#else
#define LV_ASSERT_OBJ(obj_p, obj_class)                                                                                \
    do {                                                                                                               \
