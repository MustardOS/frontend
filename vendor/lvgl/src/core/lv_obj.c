...
static int obj_valid_child(const lv_obj_t *parent, const lv_obj_t *obj_to_find);
...
void lv_init(void) {
    ...
    int big_endian = endianess_test_p[0] == 0x11 ? 1 : 0;

    if (big_endian) {
        LV_ASSERT_MSG(
            LV_BIG_ENDIAN_SYSTEM == 1, "It's a big endian system but LV_BIG_ENDIAN_SYSTEM is not enabled in lv_conf.h"
        );
    } else {
        LV_ASSERT_MSG(
            LV_BIG_ENDIAN_SYSTEM == 0, "It's a little endian system but LV_BIG_ENDIAN_SYSTEM is enabled in lv_conf.h"
        );
    }
...
void lv_obj_add_flag(lv_obj_t *obj, lv_obj_flag_t f) {
    ...
    int was_on_layout = lv_obj_is_layout_positioned(obj);
...
int lv_obj_has_flag(const lv_obj_t *obj, lv_obj_flag_t f) {
    LV_ASSERT_OBJ(obj, MY_CLASS);

    return (obj->flags & f) == f ? 1 : 0;
}

int lv_obj_has_flag_any(const lv_obj_t *obj, lv_obj_flag_t f) {
    LV_ASSERT_OBJ(obj, MY_CLASS);

    return (obj->flags & f) ? 1 : 0;
}
...
int lv_obj_has_state(const lv_obj_t *obj, lv_state_t state) {
    LV_ASSERT_OBJ(obj, MY_CLASS);

    return obj->state & state ? 1 : 0;
}
...
int lv_obj_check_type(const lv_obj_t *obj, const lv_obj_class_t *class_p) {
    if (obj == NULL) return 0;
    return obj->class_p == class_p ? 1 : 0;
}

int lv_obj_has_class(const lv_obj_t *obj, const lv_obj_class_t *class_p) {
    const lv_obj_class_t *obj_class = obj->class_p;
    while (obj_class) {
        if (obj_class == class_p) return 1;
...
int lv_obj_is_valid(const lv_obj_t *obj) {
    lv_disp_t *disp = lv_disp_get_next(NULL);
    while (disp) {
        uint32_t i;
        for (i = 0; i < disp->screen_cnt; i++) {
            if (disp->screens[i] == obj) return 1;
            int found = obj_valid_child(disp->screens[i], obj);
            if (found) return 1;
        }
...
static void lv_obj_destructor(const lv_obj_class_t *class_p, lv_obj_t *obj) {
    ...
    /*Remove all style*/
    lv_obj_enable_style_refresh(0); /*No need to refresh the style because the object will be deleted*/
    lv_obj_remove_style_all(obj);
    lv_obj_enable_style_refresh(1);
...
static void lv_obj_draw(lv_event_t *e) {
    ...
    if (code == LV_EVENT_COVER_CHECK) {
        ...
        if (_lv_area_is_in(info->area, &coords, r) == 0) {
        ...
        int clip_corner = (lv_obj_get_style_clip_corner(obj, LV_PART_MAIN) && draw_dsc.radius != 0) ? 1 : 0;
        ...
        if (clip_corner) {
            ...
            lv_draw_mask_radius_init(mp, &obj->coords, draw_dsc.radius, 0);
...
static int obj_valid_child(const lv_obj_t *parent, const lv_obj_t *obj_to_find) {
    ...
    for (i = 0; i < child_cnt; i++) {
        ...
        /*Check the children*/
        int found = obj_valid_child(child, obj_to_find);
        if (found) {
            return 1;
        }
    }
