...
static plutovg_surface_t *plutovg_surface_create_uninitialized(size_t width, size_t height) {
    ...
    if (width <= 0 || height <= 0 || width >= kMaxSize || height >= kMaxSize) return NULL;
...
plutovg_surface_t *plutovg_surface_create(int width, int height) {
    ...
    if (surface) memset(surface->data, 0, surface->height * surface->stride);
    return surface;
}
...
static plutovg_surface_t *plutovg_surface_load_from_image(stbi_uc *image, int width, int height) {
    plutovg_surface_t *surface = plutovg_surface_create_uninitialized(width, height);
    if (surface) plutovg_convert_rgba_to_argb(surface->data, image, surface->width, surface->height, surface->stride);
...
plutovg_surface_t *plutovg_surface_load_from_image_base64(const char *data, int length) {
    ...
    for (int i = 0; i < length; ++i) {
        ...
        if (cc == '=') {
            ...
            if (equals_sign_count > 0) goto cleanup;
    ...
    if (output_length == 0 || equals_sign_count > 2 || (output_length % 4) == 1) goto cleanup;
...
bool plutovg_surface_write_to_png_stream(
    const plutovg_surface_t *surface, plutovg_write_func_t write_func, void *closure
) {
    plutovg_surface_write_begin(surface);
    int success =
        stbi_write_png_to_func(write_func, closure, surface->width, surface->height, 4, surface->data, surface->stride);
    plutovg_surface_write_end(surface);
    return success;
}

bool plutovg_surface_write_to_jpg_stream(
    const plutovg_surface_t *surface, plutovg_write_func_t write_func, void *closure, int quality
) {
    plutovg_surface_write_begin(surface);
    int success =
        stbi_write_jpg_to_func(write_func, closure, surface->width, surface->height, 4, surface->data, quality);
    plutovg_surface_write_end(surface);
    return success;
}
