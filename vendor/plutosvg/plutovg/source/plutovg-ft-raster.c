...
static PCell gray_find_cell(RAS_ARG) {
    ...
    if (ras.num_cells >= ras.max_cells) pvg_ft_longjmp(ras.jump_buffer, 1);
...
static void gray_render_line(RAS_ARG_ TPos from_x, TPos from_y, TPos to_x, TPos to_y) {
    ...
    if ((ey1 >= ras.max_ey && ey2 >= ras.max_ey) || (ey1 < ras.min_ey && ey2 < ras.min_ey)) return;
...
static int gray_clip_flags(RAS_ARG_ TPos x, TPos y) {
    return ((x > ras.clip_box.xMax) << 0) | ((y > ras.clip_box.yMax) << 1) | ((x < ras.clip_box.xMin) << 2)
           | ((y < ras.clip_box.yMin) << 3);
}
...
static void gray_render_conic(RAS_ARG_ const PVG_FT_Vector *control, const PVG_FT_Vector *to) {
    ...
    if ((TRUNC(arc[0].y) >= ras.max_ey && TRUNC(arc[1].y) >= ras.max_ey && TRUNC(arc[2].y) >= ras.max_ey)
        ...
        if (ras.clipping) ras.clip_flags = gray_clip_flags(RAS_VAR_ arc[0].x, arc[0].y);
        ras.x = arc[0].x;
        ras.y = arc[0].y;
        return;
    }
...
gray_render_cubic(RAS_ARG_ const PVG_FT_Vector *control1, const PVG_FT_Vector *control2, const PVG_FT_Vector *to) {
    ...
    for (;;) {
        ...
        if (dx1 * (dx1 - dx) + dy1 * (dy1 - dy) > 0 || dx2 * (dx2 - dx) + dy2 * (dy2 - dy) > 0) goto Split;

        gray_line_to(RAS_VAR_ arc[0].x, arc[0].y);

        if (arc == bez_stack) return;
...
static int gray_move_to(const PVG_FT_Vector *to, PWorker worker) {
    ...
    if (!ras.invalid) gray_record_cell(worker);
    ...
    if (ras.clipping) ras.clip_flags = gray_clip_flags(worker, x, y);
    ras.x = x;
    ras.y = y;
    return 0;
}
...
static void gray_sweep(RAS_ARG) {
    ...
    for (yindex = 0; yindex < ras.ycount; yindex++) {
        ...
        for (; cell != NULL; cell = cell->next) {
            ...
            if (cell->x > x && cover != 0) gray_hline(RAS_VAR_ x, yindex, cover * (ONE_PIXEL * 2), cell->x - x);

            cover += cell->cover;
            area = cover * (ONE_PIXEL * 2) - cell->area;

            if (area != 0 && cell->x >= 0) gray_hline(RAS_VAR_ cell->x, yindex, area, 1);

            x = cell->x + 1;
        }

        if (ras.count_ex > x && cover != 0) gray_hline(RAS_VAR_ x, yindex, cover * (ONE_PIXEL * 2), ras.count_ex - x);
    }
}
...
static int gray_convert_glyph(RAS_ARG) {
    ...
    if (ras.band_shoot > 8 && ras.band_size > 16) ras.band_size = ras.band_size / 2;
...
static int gray_raster_render(RAS_ARG_ void *buffer, long buffer_size, const PVG_FT_Raster_Params *params) {
    ...
    if (outline == NULL) return ErrRaster_Invalid_Outline;
    ...
    if (outline->n_points != outline->contours[outline->n_contours - 1] + 1) return ErrRaster_Invalid_Outline;

    /* this version does not support monochrome rendering */
    if (!(params->flags & PVG_FT_RASTER_FLAG_AA)) return ErrRaster_Invalid_Mode;

    if (!(params->flags & PVG_FT_RASTER_FLAG_DIRECT)) return ErrRaster_Invalid_Mode;
...
void PVG_FT_Raster_Render(const PVG_FT_Raster_Params *params) {
    ...
    while (error == ErrRaster_OutOfMemory) {
        length *= 2;
        if (length > PVG_FT_MAXIMUM_POOL_SIZE) break;
        void *heap = malloc(length);
        if (heap == NULL) break;
        if (worker.skip_spans < 0) rendered_spans += -worker.skip_spans;
