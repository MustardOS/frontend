...
static void plutovg_span_buffer_update_extents(plutovg_span_buffer_t *span_buffer) {
    if (span_buffer->w != -1 && span_buffer->h != -1) return;
...
void plutovg_span_buffer_intersect(
    plutovg_span_buffer_t *span_buffer, const plutovg_span_buffer_t *a, const plutovg_span_buffer_t *b
) {
    plutovg_span_buffer_reset(span_buffer);
...
static PVG_FT_Outline *ft_outline_create(int points, int contours) {
    ...
    PVG_FT_Outline *outline =
        malloc(points_size + tags_size + contours_size + contours_flag_size + sizeof(PVG_FT_Outline));
...
static PVG_FT_Outline *ft_outline_convert_stroke(
    const plutovg_path_t *path, const plutovg_matrix_t *matrix, const plutovg_stroke_data_t *stroke_data
);

static PVG_FT_Outline *ft_outline_convert(
    const plutovg_path_t *path, const plutovg_matrix_t *matrix, const plutovg_stroke_data_t *stroke_data
) {
    if (stroke_data) {
    ...
    while (plutovg_path_iterator_has_next(&it)) {
        switch (plutovg_path_iterator_next(&it, points)) {
            ...
            case PLUTOVG_PATH_COMMAND_CUBIC_TO:
                ...
                ft_outline_cubic_to(
                    outline, points[0].x, points[0].y, points[1].x, points[1].y, points[2].x, points[2].y
                );
                break;
...
static PVG_FT_Outline *ft_outline_convert_dash(
    const plutovg_path_t *path, const plutovg_matrix_t *matrix, const plutovg_stroke_dash_t *stroke_dash
) {
    if (stroke_dash->array.size == 0) return ft_outline_convert(path, matrix, NULL);
    plutovg_path_t *dashed =
        plutovg_path_clone_dashed(path, stroke_dash->offset, stroke_dash->array.data, stroke_dash->array.size);
...
void plutovg_rasterize(
    plutovg_span_buffer_t *span_buffer, const plutovg_path_t *path, const plutovg_matrix_t *matrix,
    const plutovg_rect_t *clip_rect, const plutovg_stroke_data_t *stroke_data, plutovg_fill_rule_t winding
) {
