...
#define PLUTOVG_DEFAULT_STROKE_STYLE                                                                                   \
    ((plutovg_stroke_style_t) {1.f, PLUTOVG_LINE_CAP_BUTT, PLUTOVG_LINE_JOIN_MITER, 10.f})
...
void plutovg_canvas_restore(plutovg_canvas_t *canvas) {
    if (canvas->state->next == NULL) return;
...
void plutovg_canvas_set_linear_gradient(
    plutovg_canvas_t *canvas, float x1, float y1, float x2, float y2, plutovg_spread_method_t spread,
...
void plutovg_canvas_set_radial_gradient(
    plutovg_canvas_t *canvas, float cx, float cy, float cr, float fx, float fy, float fr,
...
void plutovg_canvas_set_texture(
    plutovg_canvas_t *canvas, plutovg_surface_t *surface, plutovg_texture_type_t type, float opacity,
...
plutovg_paint_t *plutovg_canvas_get_paint(const plutovg_canvas_t *canvas, plutovg_color_t *color) {
    if (color) *color = canvas->state->color;
...
void plutovg_canvas_add_font_face(
    plutovg_canvas_t *canvas, const char *family, bool bold, bool italic, plutovg_font_face_t *face
) {
    if (canvas->face_cache == NULL) canvas->face_cache = plutovg_font_face_cache_create();
    plutovg_font_face_cache_add(canvas->face_cache, family, bold, italic, face);
}

bool plutovg_canvas_add_font_file(
    plutovg_canvas_t *canvas, const char *family, bool bold, bool italic, const char *filename, int ttcindex
) {
    if (canvas->face_cache == NULL) canvas->face_cache = plutovg_font_face_cache_create();
...
bool plutovg_canvas_select_font_face(plutovg_canvas_t *canvas, const char *family, bool bold, bool italic) {
    if (canvas->face_cache == NULL) return false;
...
int plutovg_canvas_get_dash_array(const plutovg_canvas_t *canvas, const float **dashes) {
    if (dashes) *dashes = canvas->state->stroke.dash.array.data;
...
void plutovg_canvas_arc_to(
    plutovg_canvas_t *canvas, float rx, float ry, float angle, bool large_arc_flag, bool sweep_flag, float x, float y
) {
...
void plutovg_canvas_stroke_extents(plutovg_canvas_t *canvas, plutovg_rect_t *extents) {
    plutovg_rasterize(
        &canvas->fill_spans, canvas->path, &canvas->state->matrix, NULL, &canvas->state->stroke,
...
void plutovg_canvas_fill_preserve(plutovg_canvas_t *canvas) {
    plutovg_rasterize(
        &canvas->fill_spans, canvas->path, &canvas->state->matrix, &canvas->clip_rect, NULL, canvas->state->winding
    );
    if (canvas->state->clipping) {
...
void plutovg_canvas_stroke_preserve(plutovg_canvas_t *canvas) {
    plutovg_rasterize(
        &canvas->fill_spans, canvas->path, &canvas->state->matrix, &canvas->clip_rect, &canvas->state->stroke,
        PLUTOVG_FILL_RULE_NON_ZERO
    );
    if (canvas->state->clipping) {
        plutovg_span_buffer_intersect(&canvas->clip_spans, &canvas->fill_spans, &canvas->state->clip_spans);
...
void plutovg_canvas_clip_preserve(plutovg_canvas_t *canvas) {
    if (canvas->state->clipping) {
        ...
        plutovg_rasterize(
            &canvas->state->clip_spans, canvas->path, &canvas->state->matrix, &canvas->clip_rect, NULL,
            canvas->state->winding
        );
...
float plutovg_canvas_add_text(
    plutovg_canvas_t *canvas, const void *text, int length, plutovg_text_encoding_t encoding, float x, float y
) {
    plutovg_state_t *state = canvas->state;
    if (state->font_face == NULL || state->font_size <= 0.f) return 0.f;
    ...
    while (plutovg_text_iterator_has_next(&it)) {
        ...
        advance_width += plutovg_font_face_get_glyph_path(
            state->font_face, state->font_size, x + advance_width, y, codepoint, canvas->path
        );
    }
...
float plutovg_canvas_fill_text(
    plutovg_canvas_t *canvas, const void *text, int length, plutovg_text_encoding_t encoding, float x, float y
) {
    plutovg_canvas_new_path(canvas);
...
void plutovg_canvas_font_metrics(
    const plutovg_canvas_t *canvas, float *ascent, float *descent, float *line_gap, plutovg_rect_t *extents
) {
    plutovg_state_t *state = canvas->state;
    if (state->font_face && state->font_size > 0.f) {
...
void plutovg_canvas_glyph_metrics(
    plutovg_canvas_t *canvas, plutovg_codepoint_t codepoint, float *advance_width, float *left_side_bearing,
    plutovg_rect_t *extents
) {
    plutovg_state_t *state = canvas->state;
    if (state->font_face && state->font_size > 0.f) {
        plutovg_font_face_get_glyph_metrics(
            state->font_face, state->font_size, codepoint, advance_width, left_side_bearing, extents
        );
        return;
    }
...
float plutovg_canvas_text_extents(
    plutovg_canvas_t *canvas, const void *text, int length, plutovg_text_encoding_t encoding, plutovg_rect_t *extents
) {
    plutovg_state_t *state = canvas->state;
    if (state->font_face && state->font_size > 0.f) {
