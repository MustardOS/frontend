...
void plutovg_matrix_map_points(
    const plutovg_matrix_t *matrix, const plutovg_point_t *src, plutovg_point_t *dst, int count
) {
...
static int parse_matrix_parameters(const char **begin, const char *end, float values[6], int required, int optional) {
    if (!plutovg_skip_ws_and_delim(begin, end, '(')) return 0;
    int count = 0;
    int max_count = required + optional;
    bool has_trailing_comma = false;
    for (; count < max_count; ++count) {
        if (!plutovg_parse_number(begin, end, values + count)) break;
...
bool plutovg_matrix_parse(plutovg_matrix_t *matrix, const char *data, int length) {
    ...
    while (it < end) {
        if (plutovg_skip_string(&it, end, "matrix")) {
            ...
            if (count == 0) return false;
            if (count == 3) plutovg_matrix_translate(matrix, values[1], values[2]);
