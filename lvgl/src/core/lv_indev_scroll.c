...
static lv_coord_t
elastic_diff(lv_obj_t *scroll_obj, lv_coord_t diff, lv_coord_t scroll_start, lv_coord_t scroll_end, lv_dir_t dir);
...
void _lv_indev_scroll_throw_handler(_lv_indev_proc_t *proc) {
    ...
    if (lv_obj_has_flag(scroll_obj, LV_OBJ_FLAG_SCROLL_MOMENTUM) == 0) {
    ...
    if (proc->types.pointer.scroll_dir == LV_DIR_VER) {
        ...
        if (align_y == LV_SCROLL_SNAP_NONE) {
            ...
            proc->types.pointer.scroll_throw_vect.y =
                elastic_diff(scroll_obj, proc->types.pointer.scroll_throw_vect.y, st, sb, LV_DIR_VER);
        ...
        if (align_x == LV_SCROLL_SNAP_NONE) {
            ...
            proc->types.pointer.scroll_throw_vect.x =
                elastic_diff(scroll_obj, proc->types.pointer.scroll_throw_vect.x, sl, sr, LV_DIR_HOR);
...
static lv_obj_t *find_scroll_obj(_lv_indev_proc_t *proc) {
    ...
    while (obj_act) {
        ...
        if (lv_obj_has_flag(obj_act, LV_OBJ_FLAG_SCROLLABLE) == 0) {
            /*If this object don't want to chain the scroll to the parent stop searching*/
            if (lv_obj_has_flag(obj_act, LV_OBJ_FLAG_SCROLL_CHAIN_HOR) == 0 && hor_en) break;
            if (lv_obj_has_flag(obj_act, LV_OBJ_FLAG_SCROLL_CHAIN_VER) == 0 && ver_en) break;
        ...
        if ((scroll_dir & LV_DIR_LEFT) == 0) left_en = 0;
        if ((scroll_dir & LV_DIR_RIGHT) == 0) right_en = 0;
        if ((scroll_dir & LV_DIR_TOP) == 0) up_en = 0;
        if ((scroll_dir & LV_DIR_BOTTOM) == 0) down_en = 0;
...
static void init_scroll_limits(_lv_indev_proc_t *proc) {
    ...
    if (lv_obj_has_flag(obj, LV_OBJ_FLAG_SCROLL_ONE) == 0) {
...
elastic_diff(lv_obj_t *scroll_obj, lv_coord_t diff, lv_coord_t scroll_start, lv_coord_t scroll_end, lv_dir_t dir) {
    if (lv_obj_has_flag(scroll_obj, LV_OBJ_FLAG_SCROLL_ELASTIC)) {
        ...
        if (dir == LV_DIR_HOR) {
            ...
            switch (snap) {
                case LV_SCROLL_SNAP_CENTER:
                    snap_point = pad_left + (lv_area_get_width(&scroll_obj->coords) - pad_left - pad_right) / 2
            ...
            switch (snap) {
                case LV_SCROLL_SNAP_CENTER:
                    snap_point = pad_top + (lv_area_get_height(&scroll_obj->coords) - pad_top - pad_bottom) / 2
