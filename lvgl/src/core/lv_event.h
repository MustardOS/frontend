 ...
 *   - If already set to `0` an other event wants that point non clickable. If you want to respect it leave it as `0` or
 * set `1` to overwrite it.
 *   - If already set `1` and `point` shouldn't be clickable set to `0`
 *   - If already set to `1` you agree that `point` can click the object leave it as `1`
 */
typedef struct {
    const lv_point_t *point; /**< A point relative to screen to check if it can click the object or not*/
    int res;                 /**< 1: `point` can click the object; 0: it cannot*/
} lv_hit_test_info_t;
 ...
 */
struct _lv_event_dsc_t *
lv_obj_add_event_cb(struct _lv_obj_t *obj, lv_event_cb_t event_cb, lv_event_code_t filter, void *user_data);
 ...
 * @return          1 if any event handlers were removed
 */
int lv_obj_remove_event_cb(struct _lv_obj_t *obj, lv_event_cb_t event_cb);
 ...
 * @return                  1 if any event handlers were removed
 */
int lv_obj_remove_event_cb_with_user_data(struct _lv_obj_t *obj, lv_event_cb_t event_cb, const void *event_user_data);
 ...
 * @return          1 if any event handlers were removed
 */
int lv_obj_remove_event_dsc(struct _lv_obj_t *obj, struct _lv_event_dsc_t *event_dsc);
