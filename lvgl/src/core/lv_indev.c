...
static int indev_reset_check(_lv_indev_proc_t *proc);
...
void lv_indev_enable(lv_indev_t *indev, int en) {
...
lv_dir_t lv_indev_get_scroll_dir(const lv_indev_t *indev) {
    ...
    if (indev->driver->type != LV_INDEV_TYPE_POINTER && indev->driver->type != LV_INDEV_TYPE_BUTTON) return 0;
...
lv_obj_t *lv_indev_search_obj(lv_obj_t *obj, lv_point_t *point) {
    ...
    lv_obj_transform_point(obj, &p_trans, 0, 1);

    int hit_test_ok = lv_obj_hit_test(obj, &p_trans);
...
static void indev_keypad_proc(lv_indev_t *i, lv_indev_data_t *data) {
    ...
    int dis = lv_obj_has_state(indev_obj_act, LV_STATE_DISABLED);
    ...
    if (data->state == LV_INDEV_STATE_PRESSED && prev_state == LV_INDEV_STATE_RELEASED) {
        ...
        if (data->key == LV_KEY_NEXT) {
            lv_group_set_editing(g, 0); /*Editing is not used by KEYPAD is be sure it is disabled*/
            ...
            if (data->key == LV_KEY_ENTER) {
                ...
                if (editable_or_scrollable) {
                    ...
                    if (lv_group_get_obj_count(g) > 1) {
                        ...
                        lv_group_set_editing(g, lv_group_get_editing(g) ? 0 : 1); /*Toggle edit mode on long press*/
        ...
        if (data->key == LV_KEY_ENTER) {
            ...
            /*If the focused object is editable and now in navigate mode then on enter switch edit
               mode*/
                ...
                lv_group_set_editing(g, 1); /*Set edit mode*/
            }
        }
...
static void indev_proc_press(_lv_indev_proc_t *proc) {
    ...
    else if (proc->types.pointer.scroll_obj == NULL && lv_obj_has_flag(indev_obj_act, LV_OBJ_FLAG_PRESS_LOCK) == 0) {
        indev_obj_act = lv_indev_search_obj(lv_disp_get_layer_sys(disp), &proc->types.pointer.act_point);
        if (indev_obj_act == NULL)
            indev_obj_act = lv_indev_search_obj(lv_disp_get_layer_top(disp), &proc->types.pointer.act_point);
        if (indev_obj_act == NULL)
    ...
    if (indev_obj_act != proc->types.pointer.act_obj) {
        ...
        if (proc->types.pointer.act_obj != NULL) {
            /*Save the obj because in special cases `act_obj` can change in the Call the ancestor's event handler
             * function*/
    ...
    proc->types.pointer.scroll_throw_vect.x =
        (proc->types.pointer.scroll_throw_vect.x + proc->types.pointer.vect.x) / 2;
    proc->types.pointer.scroll_throw_vect.y =
        (proc->types.pointer.scroll_throw_vect.y + proc->types.pointer.vect.y) / 2;
...
static void indev_click_focus(_lv_indev_proc_t *proc) {
    /*Handle click focus*/
    if (lv_obj_has_flag(indev_obj_act, LV_OBJ_FLAG_CLICK_FOCUSABLE) == 0
 ...
 * @param proc pointer to an input device 'proc'
 * @return 1 if indev query should be immediately truncated.
 */
static int indev_reset_check(_lv_indev_proc_t *proc) {
    if (proc->reset_query) {
        indev_obj_act = NULL;
