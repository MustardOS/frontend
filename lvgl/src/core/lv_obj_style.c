...
static int trans_del(lv_obj_t *obj, lv_part_t part, lv_style_prop_t prop, trans_t *tr_limit);
...
void lv_obj_refresh_style(lv_obj_t *obj, lv_style_selector_t selector, lv_style_prop_t prop) {
    ...
    int is_layout_refr = lv_style_prop_has_flag(prop, LV_STYLE_PROP_LAYOUT_REFR);
    int is_ext_draw = lv_style_prop_has_flag(prop, LV_STYLE_PROP_EXT_DRAW);
    int is_inheritable = lv_style_prop_has_flag(prop, LV_STYLE_PROP_INHERIT);
    int is_layer_refr = lv_style_prop_has_flag(prop, LV_STYLE_PROP_LAYER_REFR);
...
void lv_obj_enable_style_refresh(int en) {
    style_refr = en;
}

lv_style_value_t lv_obj_get_style_prop(const lv_obj_t *obj, lv_part_t part, lv_style_prop_t prop) {
    lv_style_value_t value_act;
    int inheritable = lv_style_prop_has_flag(prop, LV_STYLE_PROP_INHERIT);
...
void lv_obj_set_local_style_prop(
    lv_obj_t *obj, lv_style_prop_t prop, lv_style_value_t value, lv_style_selector_t selector
) {
...
void lv_obj_set_local_style_prop_meta(
    lv_obj_t *obj, lv_style_prop_t prop, uint16_t meta, lv_style_selector_t selector
) {
    lv_style_t *style = get_local_style(obj, selector);
...
lv_style_res_t lv_obj_get_local_style_prop(
    lv_obj_t *obj, lv_style_prop_t prop, lv_style_value_t *value, lv_style_selector_t selector
) {
    uint32_t i;
    for (i = 0; i < obj->style_cnt; i++) {
...
int lv_obj_remove_local_style_prop(lv_obj_t *obj, lv_style_prop_t prop, lv_style_selector_t selector) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
...
_lv_style_state_cmp_t _lv_obj_style_state_compare(lv_obj_t *obj, lv_state_t state1, lv_state_t state2) {
    ...
    for (i = 0; i < obj->style_cnt; i++) {
        ...
        int valid1 = state_act & (~state1) ? 0 : 1;
        int valid2 = state_act & (~state2) ? 0 : 1;
        if (valid1 != valid2) {
...
static lv_style_res_t get_prop_core(const lv_obj_t *obj, lv_part_t part, lv_style_prop_t prop, lv_style_value_t *v) {
    ...
    int skip_trans = obj->skip_trans;
    uint32_t i;
    lv_style_res_t found;
    for (i = 0; i < obj->style_cnt; i++) {
        _lv_obj_style_t *obj_style = &obj->styles[i];
        if (obj_style->is_trans == 0) break;
