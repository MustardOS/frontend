    ...
    while (i < txt_length) {
        ...
        if (lv_txt_is_arabic_vowel(ch_enc[i])) {  // Current character is a vowel
            ...
            idx_next = lv_ap_get_char_index(
                    ch_enc[i + 2]); // Skip the vowel character to join with the character after it
        }
