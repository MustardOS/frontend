...
static void lv_style_set_prop_helper(
    lv_style_prop_t prop, lv_style_value_t value, uint16_t *prop_storage, lv_style_value_t *value_storage
);
static void lv_style_set_prop_meta_helper(
    lv_style_prop_t prop, lv_style_value_t value, uint16_t *prop_storage, lv_style_value_t *value_storage
);
...
int lv_style_remove_prop(lv_style_t *style, lv_style_prop_t prop) {
    LV_ASSERT_STYLE(style);
    ...
    for (i = 0; i < style->prop_cnt; i++) {
        if (LV_STYLE_PROP_ID_MASK(old_props[i]) == prop) {
            ...
            if (style->prop_cnt == 2) {
                ...
                if (new_values_and_props == NULL) return 0;
...
void lv_style_transition_dsc_init(
    lv_style_transition_dsc_t *tr, const lv_style_prop_t props[], lv_anim_path_cb_t path_cb, uint32_t time,
...
int lv_style_is_empty(const lv_style_t *style) {
    LV_ASSERT_STYLE(style);

    return style->prop_cnt == 0 ? 1 : 0;
}
...
uint8_t _lv_style_prop_lookup_flags(lv_style_prop_t prop) {
    ...
    if (prop < _LV_STYLE_NUM_BUILT_IN_PROPS) return _lv_style_builtin_prop_flag_lookup_table[prop];
