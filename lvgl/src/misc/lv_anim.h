 ...
 * @param a         pointer to an initialized `lv_anim_t` variable
 * @param en        1: apply the start value immediately in `lv_anim_start`;
 *                  0: apply the start value only when `delay` ms is elapsed and the animations really starts
 */
static inline void lv_anim_set_early_apply(lv_anim_t *a, int en) {
    a->early_apply = en;
}
 ...
 * @return          1: at least 1 animation is deleted, 0: no animation is deleted
 */
 ...
 * @return          1: at least 1 animation is deleted, 0: no animation is deleted
 */
static inline int lv_anim_custom_del(lv_anim_t *a, lv_anim_custom_exec_cb_t exec_cb) {
