...
void _lv_log_add(lv_log_level_t level, const char *file, int line, const char *func, const char *format, ...) {
    ...
    if (level >= LV_LOG_LEVEL) {
...
#if LV_LOG_PRINTF
        printf(
            "[%s]\t(%" LV_PRId32 ".%03" LV_PRId32 ", +%" LV_PRId32 ")\t %s: ", lvl_prefix[level], t / 1000, t % 1000,
            t - last_log_time, func
        );
        vprintf(format, args);
        printf(" \t(in %s line #%d)\n", &file[p], line);
#else
        if (custom_print_cb) {
            ...
            lv_snprintf(
                buf, sizeof(buf),
                "[%s]\t(%" LV_PRId32 ".%03" LV_PRId32 ", +%" LV_PRId32 ")\t %s: %s \t(in %s line #%d)\n",
            ...
            lv_snprintf(
                buf, sizeof(buf),
                "[%s]\t(%" LV_PRId32 ".%03" LV_PRId32 ", +%" LV_PRId32 ")\t %s: %pV \t(in %s line #%d)\n",
