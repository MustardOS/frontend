 ...
 *  STATIC PROTOTYPES
 **********************/
static int lv_timer_exec(lv_timer_t *timer);
static uint32_t lv_timer_time_remaining(lv_timer_t *timer);

/**********************
 *  STATIC VARIABLES
 **********************/
static int lv_timer_run = 0;
static uint8_t idle_last = 0;
static int timer_deleted;
...
uint32_t LV_ATTRIBUTE_TIMER_HANDLER lv_timer_handler(void) {
    ...
    if (lv_timer_run == 0) {
        already_running = 0; /*Release mutex*/
        return 1;
    }
    ...
    while (next) {
        if (!next->paused) {
            ...
            if (delay < time_till_next) time_till_next = delay;
        }
    ...
    already_running = 0; /*Release the mutex*/
 ...
 * @return 1: execute, 0: not executed
 */
static int lv_timer_exec(lv_timer_t *timer) {
    ...
    if (timer_deleted == 0) {           /*The timer might be deleted by itself as well*/
        if (timer->repeat_count == 0) { /*The repeat count is over, delete the timer*/
            TIMER_TRACE(
                "deleting timer with %p callback because the repeat count is over", *((void **) &timer->timer_cb)
            );
            lv_timer_del(timer);
        }
    }
