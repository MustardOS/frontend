 ...
 * @return 0: the two area has NO common parts, res_p is invalid
 */
int _lv_area_intersect(lv_area_t *res_p, const lv_area_t *a1_p, const lv_area_t *a2_p);
 ...
 * @return 0:the point is out of the area
 */
int _lv_area_is_point_on(const lv_area_t *a_p, const lv_point_t *p_p, lv_coord_t radius);

/**
 * Check if two area has common parts
 * @param a1_p pointer to an area.
 * @param a2_p pointer to an other area
 * @return 0: a1_p and a2_p has no common parts
 */
int _lv_area_is_on(const lv_area_t *a1_p, const lv_area_t *a2_p);
 ...
 * @return 1: `ain_p` is fully inside `aholder_p`
 */
int _lv_area_is_in(const lv_area_t *ain_p, const lv_area_t *aholder_p, lv_coord_t radius);
 ...
 * @param aholder_p pointer to an area which could involve 'ain_p'
 * @param radius radius of `aholder_p` (e.g. for rounded rectangle)
 * @return 1: `aout_p` is fully outside `aholder_p`
 */
int _lv_area_is_out(const lv_area_t *aout_p, const lv_area_t *aholder_p, lv_coord_t radius);
 ...
 */
int _lv_area_is_equal(const lv_area_t *a, const lv_area_t *b);
...
#define LV_COORD_IS_PX(x)   (_LV_COORD_TYPE(x) == _LV_COORD_TYPE_PX || _LV_COORD_TYPE(x) == _LV_COORD_TYPE_PX_NEG ? 1 : 0)
#define LV_COORD_IS_SPEC(x) (_LV_COORD_TYPE(x) == _LV_COORD_TYPE_SPEC ? 1 : 0)

#define LV_COORD_SET_SPEC(x) ((x) | _LV_COORD_TYPE_SPEC)

/*Special coordinates*/
#define LV_PCT(x)           (x < 0 ? LV_COORD_SET_SPEC(1000 - (x)) : LV_COORD_SET_SPEC(x))
#define LV_COORD_IS_PCT(x)  ((LV_COORD_IS_SPEC(x) && _LV_COORD_PLAIN(x) <= 2000) ? 1 : 0)
