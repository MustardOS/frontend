...
lv_anim_timeline_t *lv_anim_timeline_create(void) {
    lv_anim_timeline_t *at = lv_mem_alloc(sizeof(lv_anim_timeline_t));
...
void lv_anim_timeline_set_reverse(lv_anim_timeline_t *at, int reverse) {
    LV_ASSERT_NULL(at);
    at->reverse = reverse;
}
...
uint32_t lv_anim_timeline_get_playtime(lv_anim_timeline_t *at) {
    ...
    for (uint32_t i = 0; i < at->anim_dsc_cnt; i++) {
        ...
        if (end == LV_ANIM_PLAYTIME_INFINITE) return end;
...
int lv_anim_timeline_get_reverse(lv_anim_timeline_t *at) {
    LV_ASSERT_NULL(at);
    return at->reverse;
}
