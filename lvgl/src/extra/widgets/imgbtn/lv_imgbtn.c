 ...
 * to a file)
 */
void lv_imgbtn_set_src(
    lv_obj_t *obj, lv_imgbtn_state_t state, const void *src_left, const void *src_mid, const void *src_right
) {
...
static void lv_imgbtn_event(const lv_obj_class_t *class_p, lv_event_t *e) {
    ...
    if (code == LV_EVENT_PRESSED || code == LV_EVENT_RELEASED || code == LV_EVENT_PRESS_LOST) {
        ...
        if (imgbtn->img_src_left[state] == NULL && imgbtn->img_src_mid[state] != NULL
