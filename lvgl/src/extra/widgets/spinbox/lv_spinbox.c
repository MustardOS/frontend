 ...
 * @param b 1 or 0 to enable or disable (default)
 */
void lv_spinbox_set_rollover(lv_obj_t *obj, int b) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_spinbox_t *spinbox = (lv_spinbox_t *) obj;
 ...
 * @param obj pointer to spinbox
 */
int lv_spinbox_get_rollover(lv_obj_t *obj) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_spinbox_t *spinbox = (lv_spinbox_t *) obj;
...
static void lv_spinbox_constructor(const lv_obj_class_t *class_p, lv_obj_t *obj) {
    ...
    lv_textarea_set_one_line(obj, 1);
    lv_textarea_set_cursor_click_pos(obj, 1);
