...
void lv_chart_set_axis_tick(
    lv_obj_t *obj, lv_chart_axis_t axis, lv_coord_t major_len, lv_coord_t minor_len, lv_coord_t major_cnt,
    lv_coord_t minor_cnt, int label_en, lv_coord_t draw_size
) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
...
void lv_chart_hide_series(lv_obj_t *chart, lv_chart_series_t *series, int hide) {
...
void lv_chart_set_value_by_id2(
    lv_obj_t *obj, lv_chart_series_t *ser, uint16_t id, lv_coord_t x_value, lv_coord_t y_value
) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
    LV_ASSERT_NULL(ser);
    lv_chart_t *chart = (lv_chart_t *) obj;
...
static void lv_chart_event(const lv_obj_class_t *class_p, lv_event_t *e) {
    ...
    if (code == LV_EVENT_PRESSED) {
        ...
        lv_event_set_ext_draw_size(
            e,
            LV_MAX4(
                chart->tick[0].draw_size, chart->tick[1].draw_size, chart->tick[2].draw_size, chart->tick[3].draw_size
            )
        );
        ...
        if (_lv_ll_is_empty(&chart->series_ll) == 0) {
            if (chart->type == LV_CHART_TYPE_LINE)
...
static void draw_div_lines(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx) {
    ...
    int mask_ret = _lv_area_intersect(&series_clip_area, &obj->coords, draw_ctx->clip_area);
    if (mask_ret == 0) return;
...
static void draw_series_line(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx) {
    lv_area_t clip_area;
    if (_lv_area_intersect(&clip_area, &obj->coords, draw_ctx->clip_area) == 0) return;
    ...
    int crowded_mode = chart->point_cnt >= w ? 1 : 0;
...
static void draw_cursors(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx) {
    ...
    _LV_LL_READ_BACK(&chart->cursor_ll, cursor) {
        ...
        lv_area_t point_area;
        int draw_point = point_w && point_h;
        if (draw_point) {
...
static void draw_y_ticks(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, lv_chart_axis_t axis) {
    ...
    for (i = 0; i <= total_tick_num; i++) {
        ...
        int32_t tick_value =
            lv_map(total_tick_num - i, 0, total_tick_num, chart->ymin[sec_axis], chart->ymax[sec_axis]);
        ...
        if (major && t->label_en) {
            ...
            lv_point_t size;
            lv_txt_get_size(
                &size, part_draw_dsc.text, label_dsc.font, label_dsc.letter_space, label_dsc.line_space, LV_COORD_MAX,
                LV_TEXT_FLAG_NONE
            );
