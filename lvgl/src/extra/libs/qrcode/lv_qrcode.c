...
const lv_obj_class_t lv_qrcode_class = {
    .constructor_cb = lv_qrcode_constructor, .destructor_cb = lv_qrcode_destructor, .base_class = &lv_canvas_class
};
...
lv_res_t lv_qrcode_update(lv_obj_t *qrcode, const void *data, uint32_t data_len) {
    ...
    if (version_extend && qr_version < qrcodegen_VERSION_MAX) {
        qr_version =
            qr_version + version_extend > qrcodegen_VERSION_MAX ? qrcodegen_VERSION_MAX : qr_version + version_extend;
    }
    ...
    int ok = qrcodegen_encodeBinary(
        data_tmp, data_len, qr0, qrcodegen_Ecc_MEDIUM, qr_version, qr_version, qrcodegen_Mask_AUTO, 1
    );
    ...
    for (y = margin; y < scaled + margin; y += scale) {
        ...
        for (x = margin; x < scaled + margin; x++) {
            int a = qrcodegen_getModule(qr0, (x - margin) / scale, (y - margin) / scale);

            if (aligned == 0 && (x & 0x7) == 0) aligned = 1;
