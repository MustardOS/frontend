...
void lv_monkey_set_enable(lv_monkey_t *monkey, int en) {
    LV_ASSERT_NULL(monkey);
    en ? lv_timer_resume(monkey->timer) : lv_timer_pause(monkey->timer);
}

int lv_monkey_get_enable(lv_monkey_t *monkey) {
    LV_ASSERT_NULL(monkey);
...
static void lv_monkey_timer_cb(lv_timer_t *timer) {
    ...
    lv_timer_set_period(
        monkey->timer, lv_monkey_random(monkey->config.period_range.min, monkey->config.period_range.max)
    );
}
