...
fragment_attach(lv_fragment_manager_t *manager, lv_fragment_t *fragment, lv_obj_t *const *container);
...
int lv_fragment_manager_send_event(lv_fragment_manager_t *manager, int code, void *userdata) {
    ...
    _LV_LL_READ_BACK(&manager->attached, p) {
        ...
        if (lv_fragment_manager_send_event(instance->child_manager, code, userdata)) return 1;
        if (p->cls->event_cb && p->cls->event_cb(instance, code, userdata)) return 1;
    }
    return 0;
}
