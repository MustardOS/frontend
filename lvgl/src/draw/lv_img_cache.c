...
static int lv_img_cache_match(const void *src1, const void *src2);
#endif
...
static int lv_img_cache_match(const void *src1, const void *src2) {
    ...
    if (src_type == LV_IMG_SRC_VARIABLE) return src1 == src2;
    if (src_type != LV_IMG_SRC_FILE) return 0;
    if (lv_img_src_get_type(src2) != LV_IMG_SRC_FILE) return 0;
