...
static void draw_bg_grad_simple(
    lv_draw_sdl_ctx_t *ctx, const lv_area_t *coords, const lv_area_t *draw_area, const lv_grad_dsc_t *grad,
    int blend_mod
);

static void draw_bg_grad_radius(
    lv_draw_sdl_ctx_t *ctx, const lv_area_t *coords, const lv_area_t *draw_area, const lv_draw_rect_dsc_t *dsc
);
...
static void
draw_border(lv_draw_sdl_ctx_t *ctx, const lv_area_t *coords, const lv_area_t *draw_area, const lv_draw_rect_dsc_t *dsc);

static void
draw_shadow(lv_draw_sdl_ctx_t *ctx, const lv_area_t *coords, const lv_area_t *clip, const lv_draw_rect_dsc_t *dsc);

static void
draw_outline(lv_draw_sdl_ctx_t *ctx, const lv_area_t *coords, const lv_area_t *clip, const lv_draw_rect_dsc_t *dsc);

static void draw_border_generic(
    lv_draw_sdl_ctx_t *ctx, const lv_area_t *outer_area, const lv_area_t *inner_area, const lv_area_t *clip,
    lv_coord_t rout, lv_coord_t rin, lv_color_t color, lv_opa_t opa, lv_blend_mode_t blend_mode
);

static void frag_render_borders(
    SDL_Renderer *renderer, SDL_Texture *frag, lv_coord_t frag_size, const lv_area_t *coords, const lv_area_t *clipped,
    int full
);

static void frag_render_center(
    SDL_Renderer *renderer, SDL_Texture *frag, lv_coord_t frag_size, const lv_area_t *coords, const lv_area_t *clipped,
    int full
);
...
rect_grad_frag_key_create(const lv_grad_dsc_t *grad, lv_coord_t w, lv_coord_t h, lv_coord_t radius);
...
rect_border_key_create(lv_coord_t rout, lv_coord_t rin, const lv_area_t *outer_area, const lv_area_t *inner_area);
...
#define SKIP_BORDER(dsc)                                                                                               \
    ((dsc)->border_opa <= LV_OPA_MIN || (dsc)->border_width == 0 || (dsc)->border_side == LV_BORDER_SIDE_NONE          \
     || (dsc)->border_post)
#define SKIP_SHADOW(dsc)                                                                                               \
    ((dsc)->shadow_width == 0 || (dsc)->shadow_opa <= LV_OPA_MIN                                                       \
     || ((dsc)->shadow_width == 1 && (dsc)->shadow_spread <= 0 && (dsc)->shadow_ofs_x == 0                             \
         && (dsc)->shadow_ofs_y == 0))
...
void lv_draw_sdl_draw_rect(lv_draw_ctx_t *draw_ctx, const lv_draw_rect_dsc_t *dsc, const lv_area_t *coords) {
    ...
    int has_content = _lv_area_intersect(&t_area, &t_coords, &t_clip);
...
SDL_Texture *lv_draw_sdl_rect_bg_frag_obtain(lv_draw_sdl_ctx_t *ctx, lv_coord_t radius, int *in_cache) {
    ...
    if (texture == NULL) {
        ...
        lv_draw_mask_radius_init(&mask_rout_param, &coords, radius, 0);
...
SDL_Texture *lv_draw_sdl_rect_grad_frag_obtain(
    lv_draw_sdl_ctx_t *ctx, const lv_grad_dsc_t *grad, lv_coord_t w, lv_coord_t h, lv_coord_t radius, int *in_cache
) {
    ...
    if (texture == NULL) {
        ...
        texture = SDL_CreateTexture(
            ctx->renderer, LV_DRAW_SDL_TEXTURE_FORMAT, SDL_TEXTUREACCESS_TARGET, lv_area_get_width(&coords),
        ...
        draw_bg_grad_simple(ctx, &blend_coords, &draw_area, grad, 1);
...
SDL_Texture *lv_draw_sdl_rect_grad_strip_obtain(lv_draw_sdl_ctx_t *ctx, const lv_grad_dsc_t *grad, int *in_cache) {
    ...
    if (texture == NULL) {
        ...
        SDL_Surface *surface = SDL_CreateRGBSurfaceFrom(
            pixels, width, height, LV_COLOR_DEPTH, width * LV_COLOR_DEPTH / 8, rmask, gmask, bmask, amask
        );
...
void lv_draw_sdl_rect_bg_frag_draw_corners(
    lv_draw_sdl_ctx_t *ctx, SDL_Texture *frag, lv_coord_t frag_size, const lv_area_t *coords, const lv_area_t *clip,
    int full
) {
    if (!clip) clip = coords;
    ...
    if (_lv_area_intersect(&dst_area, &corner_area, clip)) {
        ...
        if (full) {
            ...
            SDL_RenderCopyEx(
                ctx->renderer, frag, &src_rect, &dst_rect, 0, NULL, SDL_FLIP_HORIZONTAL | SDL_FLIP_VERTICAL
            );
        }
    }
...
static void draw_bg_color(
    ...
    if (dsc->bg_grad.dir == LV_GRAD_DIR_NONE) {
        ...
        if (radius <= 0) {
            draw_bg_grad_simple(ctx, coords, draw_area, &dsc->bg_grad, 0);
        } else {
    ...
    lv_draw_sdl_rect_bg_frag_draw_corners(ctx, texture, real_radius, coords, draw_area, 0);
    frag_render_borders(ctx->renderer, texture, real_radius, coords, draw_area, 0);
    frag_render_center(ctx->renderer, texture, real_radius, coords, draw_area, 0);
...
static void draw_bg_grad_radius(
    ...
    lv_coord_t real_radius = LV_MIN3(bg_w / 2, bg_h / 2, radius);
    int grad_texture_in_cache = 0;
    SDL_Texture *grad_texture =
        lv_draw_sdl_rect_grad_frag_obtain(ctx, &dsc->bg_grad, bg_w, bg_h, radius, &grad_texture_in_cache);
    SDL_SetTextureBlendMode(grad_texture, SDL_BLENDMODE_BLEND);

    lv_draw_sdl_rect_bg_frag_draw_corners(ctx, grad_texture, real_radius, coords, draw_area, 1);
    ...
    if (bg_w > radius * 2) {
        ...
        draw_bg_grad_simple(ctx, coords, &part_area, &dsc->bg_grad, 0);
...
static void draw_bg_img(
    ...
    if (src_type == LV_IMG_SRC_SYMBOL) {
        ...
        SDL_Texture *texture =
            lv_draw_sdl_texture_cache_get_with_userdata(ctx, key, key_size, &key_found, (void **) &cache_header);
        SDL_free(key);
        if (texture) {
            header = *cache_header;
        ...
        if (dsc->radius > 0) {
            lv_draw_mask_radius_init(&radius_param, coords, dsc->radius, 0);
            radius_mask_id = lv_draw_mask_add(&radius_param, NULL);
        }

        /*Center align*/
        if (dsc->bg_img_tiled == 0) {
            lv_area_t area;
...
draw_shadow(lv_draw_sdl_ctx_t *ctx, const lv_area_t *coords, const lv_area_t *clip, const lv_draw_rect_dsc_t *dsc) {
    ...
    lv_coord_t frag_size =
        LV_MIN3(lv_area_get_width(&core_area) / 2, lv_area_get_height(&core_area) / 2, LV_MAX(sw / 2, radius));
    ...
    if (texture == NULL) {
        ...
        lv_draw_mask_radius_param_t mask_rout_param;
        lv_draw_mask_radius_init(&mask_rout_param, &mask_area, radius, 0);
        int16_t mask_id = lv_draw_mask_add(&mask_rout_param, NULL);
        lv_opa_t *mask_buf = lv_draw_sdl_mask_dump_opa(&mask_area_blurred, &mask_id, 1);
        lv_stack_blur_grayscale(
            mask_buf, lv_area_get_width(&mask_area_blurred), lv_area_get_height(&mask_area_blurred), sw / 2 + sw % 2
        );
        texture = lv_sdl_create_opa_texture(
            ctx->renderer, mask_buf, blur_frag_size, blur_frag_size, lv_area_get_width(&mask_area_blurred)
        );
    ...
    lv_draw_sdl_rect_bg_frag_draw_corners(ctx, texture, blur_frag_size, &shadow_area, clip, 0);
    frag_render_borders(ctx->renderer, texture, blur_frag_size, &shadow_area, clip, 0);
    frag_render_center(ctx->renderer, texture, blur_frag_size, &shadow_area, clip, 0);
...
static void draw_border(
    ...
    draw_border_generic(
        ctx, coords, &area_inner, draw_area, rout, rin, dsc->border_color, dsc->border_opa, dsc->blend_mode
    );
}
...
draw_outline(lv_draw_sdl_ctx_t *ctx, const lv_area_t *coords, const lv_area_t *clip, const lv_draw_rect_dsc_t *dsc) {
    ...
    draw_border_generic(
        ctx, &area_outer, &area_inner, clip, rout, rin, dsc->outline_color, dsc->outline_opa, dsc->blend_mode
    );
}
...
static void draw_border_generic(
    ...
    if (texture == NULL) {
        ...
        lv_draw_mask_radius_param_t mask_rout_param;
        if (rout > 0) {
            lv_draw_mask_radius_init(&mask_rout_param, &frag_area, rout, 0);
        ...
        const lv_area_t frag_inner_area = {
            frag_area.x1 + key.offsets.x1, frag_area.y1 + key.offsets.y1, frag_area.x2 + key.offsets.x2,
            frag_area.y2 + key.offsets.y2
        };
        lv_draw_mask_radius_param_t mask_rin_param;
        lv_draw_mask_radius_init(&mask_rin_param, &frag_inner_area, rin, 1);
    ...
    lv_draw_sdl_rect_bg_frag_draw_corners(ctx, texture, frag_size, outer_area, clip, 1);
    frag_render_borders(renderer, texture, frag_size, outer_area, clip, 1);
