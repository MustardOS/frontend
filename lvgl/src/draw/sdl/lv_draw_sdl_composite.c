...
int lv_draw_sdl_composite_begin(
    ...
    if (!_lv_area_intersect(apply_area, &full_coords, clip_in)) return 0;
    int has_mask = lv_draw_mask_is_any(apply_area);
    ...
    if (internals->composition) {
        ...
        switch (blend_mode) {
            ...
            case LV_BLEND_MODE_SUBTRACTIVE: {
                SDL_BlendMode mode = SDL_ComposeCustomBlendMode(
                    SDL_BLENDFACTOR_ONE, SDL_BLENDFACTOR_ONE, SDL_BLENDOPERATION_SUBTRACT, SDL_BLENDFACTOR_ONE,
            ...
            case LV_BLEND_MODE_MULTIPLY: {
                SDL_BlendMode mode = SDL_ComposeCustomBlendMode(
                    SDL_BLENDFACTOR_ZERO, SDL_BLENDFACTOR_SRC_COLOR, SDL_BLENDOPERATION_ADD, SDL_BLENDFACTOR_ZERO,
