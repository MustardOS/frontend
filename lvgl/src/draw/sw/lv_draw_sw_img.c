...
static void convert_cb(
    const lv_area_t *dest_area, const void *src_buf, lv_coord_t src_w, lv_coord_t src_h, lv_coord_t src_stride,
...
void LV_ATTRIBUTE_FAST_MEM lv_draw_sw_img_decoded(
    ...
    int mask_any = lv_draw_mask_is_any(&draw_area);
    int transform = draw_dsc->angle != 0 || draw_dsc->zoom != LV_IMG_ZOOM_NONE ? 1 : 0;
    ...
    /*In the other cases every pixel need to be checked one-by-one*/
        ...
        lv_draw_mask_res_t mask_res_def =
            (cf != LV_IMG_CF_TRUE_COLOR || draw_dsc->angle || draw_dsc->zoom != LV_IMG_ZOOM_NONE)
        ...
        while (blend_area.y1 <= y_last) {
            ...
            if (transform) {
                lv_draw_transform(
                    draw_ctx, &transform_area, src_buf, src_w, src_h, src_w, draw_dsc, cf, rgb_buf, mask_buf
                );
            } else {
