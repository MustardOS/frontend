...
static void /* LV_ATTRIBUTE_FAST_MEM */ draw_letter_normal(
    lv_draw_ctx_t *draw_ctx, const lv_draw_label_dsc_t *dsc, const lv_point_t *pos, lv_font_glyph_dsc_t *g,
    const uint8_t *map_p
);

#if LV_DRAW_COMPLEX && LV_USE_FONT_SUBPX
static void draw_letter_subpx(
    lv_draw_ctx_t *draw_ctx, const lv_draw_label_dsc_t *dsc, const lv_point_t *pos, lv_font_glyph_dsc_t *g,
    const uint8_t *map_p
);
...
void lv_draw_sw_letter(
    ...
    int g_ret = lv_font_get_glyph_dsc(dsc->font, &g, letter, '\0');
    if (g_ret == 0) {
...
static void LV_ATTRIBUTE_FAST_MEM draw_letter_normal(
    ...
    int mask_any = lv_draw_mask_is_any(&mask_area);
#endif
    ...
    for (row = row_start; row < row_end; row++) {
        ...
        /*Apply masks if any*/
        if (mask_any) {
            blend_dsc.mask_res = lv_draw_mask_apply(mask_buf + mask_p_start, fill_area.x1, fill_area.y2, fill_w);
...
static void draw_letter_subpx(
    ...
    int32_t col_end =
        pos->x + box_w / 3 <= draw_ctx->clip_area->x2 ? box_w : (draw_ctx->clip_area->x2 - pos->x + 1) * 3;
    int32_t row_start = pos->y >= draw_ctx->clip_area->y1 ? 0 : draw_ctx->clip_area->y1 - pos->y;
    int32_t row_end = pos->y + box_h <= draw_ctx->clip_area->y2 ? box_h : draw_ctx->clip_area->y2 - pos->y + 1;
    ...
    mask_area.y2 = mask_area.y1 + row_end;
    int mask_any = lv_draw_mask_is_any(&map_area);
    uint8_t font_rgb[3];
    ...
    for (row = row_start; row < row_end; row++) {
        ...
        for (col = col_start; col < col_end; col++) {
            ...
            if (subpx_cnt == 3) {
...
#else
                uint8_t bg_rgb[3] = {
                    dest_buf_tmp->ch.red, (dest_buf_tmp->ch.green_h << 3) + dest_buf_tmp->ch.green_l,
                ...
                res_color.ch.red =
                    (uint32_t) ((uint16_t) txt_rgb[0] * font_rgb[2] + (bg_rgb[0] * (255 - font_rgb[2]))) >> 8;
                res_color.ch.blue =
                    (uint32_t) ((uint16_t) txt_rgb[2] * font_rgb[0] + (bg_rgb[2] * (255 - font_rgb[0]))) >> 8;
#else
                res_color.ch.red =
