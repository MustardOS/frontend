...
static lv_grad_t *next_in_cache(lv_grad_t *item) {
    ...
    if (item == NULL) return (lv_grad_t *) LV_GC_ROOT(_lv_grad_cache_mem);
...
#if LV_DITHER_ERROR_DIFFUSION == 1
        item->error_acc =
            (lv_scolor24_t *) (grad_cache_end + ALIGN(sizeof(*item)) + ALIGN(size * sizeof(lv_grad_color_t))
                               + ALIGN(map_size * sizeof(lv_color_t)));
        item->w = w;
#endif
#endif
...
lv_gradient_calculate(const lv_grad_dsc_t *dsc, lv_coord_t range, lv_coord_t frac) {
    lv_grad_color_t tmp;
    ...
    lv_grad_color_t r = GRAD_CM(
        LV_UDIV255(two.ch.red * mix + one.ch.red * imix), LV_UDIV255(two.ch.green * mix + one.ch.green * imix),
