...
void draw_border_generic(
    lv_draw_ctx_t *draw_ctx, const lv_area_t *outer_area, const lv_area_t *inner_area, lv_coord_t rout, lv_coord_t rin,
    lv_color_t color, lv_opa_t opa, lv_blend_mode_t blend_mode
);

static void draw_border_simple(
    lv_draw_ctx_t *draw_ctx, const lv_area_t *outer_area, const lv_area_t *inner_area, lv_color_t color, lv_opa_t opa
);
...
static void draw_bg(lv_draw_ctx_t *draw_ctx, const lv_draw_rect_dsc_t *dsc, const lv_area_t *coords) {
    ...
    int mask_any = lv_draw_mask_is_any(&bg_coords);
    ...
    if (rout > 0 || mask_any) {
        ...
        lv_draw_mask_radius_init(&mask_rout_param, &bg_coords, rout, 0);
    ...
    if (grad && dither_mode == LV_DITHER_NONE) {
        ...
        if (grad_dir == LV_GRAD_DIR_VER) grad_size = coords_bg_h;
    } else
    ...
    int mask_any_center = lv_draw_mask_is_any(&center_coords);
...
#if LV_DRAW_COMPLEX
static void
    LV_ATTRIBUTE_FAST_MEM draw_shadow(lv_draw_ctx_t *draw_ctx, const lv_draw_rect_dsc_t *dsc, const lv_area_t *coords) {
    ...
    int mask_any = lv_draw_mask_is_any(&shadow_area);
    int simple = 1;
    if (mask_any || dsc->bg_opa < LV_OPA_COVER || dsc->blend_mode != LV_BLEND_MODE_NORMAL) simple = 0;
    ...
    int16_t mask_rout_id = LV_MASK_ID_INV;
    if (!simple) {
        lv_draw_mask_radius_init(&mask_rout_param, &bg_area, r_bg, 1);
        mask_rout_id = lv_draw_mask_add(&mask_rout_param, NULL);
    }
 ...
 */
static void LV_ATTRIBUTE_FAST_MEM
shadow_draw_corner_buf(const lv_area_t *coords, uint16_t *sh_buf, lv_coord_t sw, lv_coord_t r) {
    int32_t sw_ori = sw;
    ...
    lv_draw_mask_radius_init(&mask_param, &sh_area, r, 0);
...
static void draw_outline(lv_draw_ctx_t *draw_ctx, const lv_draw_rect_dsc_t *dsc, const lv_area_t *coords) {
    ...
    draw_border_generic(
        draw_ctx, &area_outer, &area_inner, rout, rin, dsc->outline_color, dsc->outline_opa, dsc->blend_mode
    );
}
...
void draw_border_generic(
    ...
    int mask_any = lv_draw_mask_is_any(outer_area);
    ...
    lv_draw_mask_radius_param_t mask_rout_param;
    if (rout > 0) {
        lv_draw_mask_radius_init(&mask_rout_param, outer_area, rout, 0);
        mask_rout_id = lv_draw_mask_add(&mask_rout_param, NULL);
    }

    /*Create mask for the inner mask*/
    lv_draw_mask_radius_param_t mask_rin_param;
    lv_draw_mask_radius_init(&mask_rin_param, inner_area, rin, 1);
    ...
    int top_side = outer_area->y1 <= inner_area->y1 ? 1 : 0;
    int bottom_side = outer_area->y2 >= inner_area->y2 ? 1 : 0;
    ...
    /*No masks*/
    int left_side = outer_area->x1 <= inner_area->x1 ? 1 : 0;
    int right_side = outer_area->x2 >= inner_area->x2 ? 1 : 0;
