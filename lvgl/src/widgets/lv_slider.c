...
static void position_knob(lv_obj_t *obj, lv_area_t *knob_area, const lv_coord_t knob_size, const int hor);
static void draw_knob(lv_event_t *e);
static int is_slider_horizontal(lv_obj_t *obj);
...
int lv_slider_is_dragged(const lv_obj_t *obj) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_slider_t *slider = (lv_slider_t *) obj;

    return slider->dragging ? 1 : 0;
}
...
static void lv_slider_event(const lv_obj_class_t *class_p, lv_event_t *e) {
    ...
    if (code == LV_EVENT_HIT_TEST) {
        ...
        if ((info->res == 0) && (type == LV_SLIDER_MODE_RANGE)) {
        ...
        if (type == LV_SLIDER_MODE_NORMAL || type == LV_SLIDER_MODE_SYMMETRICAL) {
            ...
            int hor = lv_obj_get_width(obj) >= lv_obj_get_height(obj);
            ...
            if (hor) {
                if ((base_dir != LV_BASE_DIR_RTL && p.x > slider->right_knob_area.x2)
                    ...
                    slider->value_to_set = &slider->bar.cur_value;
                           ...
                           || (base_dir == LV_BASE_DIR_RTL && p.x > slider->left_knob_area.x2)) {
                    ...
                    dist_right = LV_ABS(
                        (slider->right_knob_area.x1 + (slider->right_knob_area.x2 - slider->right_knob_area.x1) / 2)
                        - p.x
                    );
                ...
                if (p.y < slider->right_knob_area.y1) {
                    ...
                    dist_right = LV_ABS(
                        (slider->right_knob_area.y1 + (slider->right_knob_area.y2 - slider->right_knob_area.y1) / 2)
                        - p.y
                    );
...
static void draw_knob(lv_event_t *e) {
    ...
    const int is_rtl = LV_BASE_DIR_RTL == lv_obj_get_style_base_dir(obj, LV_PART_MAIN);
    const int is_horizontal = is_slider_horizontal(obj);

    lv_area_t knob_area;
    lv_coord_t knob_size;
    int is_symmetrical = 0;
