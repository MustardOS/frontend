...
static int char_is_accepted(lv_obj_t *obj, uint32_t c);
...
static inline int is_valid_but_non_printable_char(const uint32_t letter);
...
void lv_textarea_add_char(lv_obj_t *obj, uint32_t c) {
    ...
    if (char_is_accepted(obj, c_uni) == 0) {
...
void lv_textarea_set_password_mode(lv_obj_t *obj, int en) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
...
void lv_textarea_set_text_selection(lv_obj_t *obj, int en) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
...
int lv_textarea_get_cursor_click_pos(lv_obj_t *obj) {
    LV_ASSERT_OBJ(obj, MY_CLASS);

    lv_textarea_t *ta = (lv_textarea_t *) obj;
    return ta->cursor.click_pos ? 1 : 0;
}

int lv_textarea_get_password_mode(const lv_obj_t *obj) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
...
const char *lv_textarea_get_password_bullet(lv_obj_t *obj) {
    ...
    if (lv_font_get_glyph_dsc(font, &g, LV_TEXTAREA_PWD_BULLET_UNICODE, 0)) return LV_SYMBOL_BULLET;
    return "*";
}

int lv_textarea_get_one_line(const lv_obj_t *obj) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
...
int lv_textarea_text_is_selected(const lv_obj_t *obj) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
...
int lv_textarea_get_text_selection(lv_obj_t *obj) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
...
static int char_is_accepted(lv_obj_t *obj, uint32_t c) {
    ...
    if (ta->accepted_chars == NULL || ta->accepted_chars[0] == '\0') return 1;
    /*Accepted character?*/
    uint32_t i = 0;

    while (ta->accepted_chars[i] != '\0') {
        uint32_t a = _lv_txt_encoded_next(ta->accepted_chars, &i);
        if (a == c) return 1; /*Accepted*/
    }

    return 0; /*The character wasn't in the list*/
}
...
static lv_res_t insert_handler(lv_obj_t *obj, const char *txt) {
    ...
    if (ta_insert_replace && ta_insert_replace[0] == '\0') return LV_RES_INV;
