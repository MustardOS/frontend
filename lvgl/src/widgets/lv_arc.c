...
static int lv_arc_angle_within_bg_bounds(lv_obj_t *obj, const uint32_t angle, const uint32_t tolerance_deg);
...
static void lv_arc_event(const lv_obj_class_t *class_p, lv_event_t *e) {
    ...
    if (code == LV_EVENT_PRESSING) {
        ...
        const int is_angle_within_bg_bounds = lv_arc_angle_within_bg_bounds(obj, (uint32_t) angle, tolerance_deg);
        ...
        if (indev_type == LV_INDEV_TYPE_ENCODER) {
            if (editing) lv_group_set_editing(g, 0);
        }
...
static void lv_arc_draw(lv_event_t *e) {
    ...
    if (arc_r > 0) {
        ...
        lv_draw_arc(
            draw_ctx, &arc_dsc, &center, part_draw_dsc.radius, arc->bg_angle_start + arc->rotation,
    ...
    if (indic_r > 0) {
        ...
        lv_draw_arc(
            draw_ctx, &arc_dsc, &center, part_draw_dsc.radius, arc->indic_angle_start + arc->rotation,
...
static void inv_arc_area(lv_obj_t *obj, uint16_t start_angle, uint16_t end_angle, lv_part_t part) {
    ...
    if (lv_obj_is_visible(obj) == 0) return;
...
static void get_center(const lv_obj_t *obj, lv_point_t *center, lv_coord_t *arc_r) {
    ...
    lv_coord_t r =
        (LV_MIN(lv_obj_get_width(obj) - left_bg - right_bg, lv_obj_get_height(obj) - top_bg - bottom_bg)) / 2;
 ...
 *
 * @return 1 if angle is within arc background bounds, 0 otherwise
 */
static int lv_arc_angle_within_bg_bounds(lv_obj_t *obj, const uint32_t angle, const uint32_t tolerance_deg) {
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_arc_t *arc = (lv_arc_t *) obj;
    ...
    else if (((smaller_angle - tolerance_deg) <= 0U) && (360U - (bigger_angle + (smaller_angle - tolerance_deg)))) {
    ...
    /* Background end angle + tolerance is bigger than 360° and bg_start_angle + tolerance is not near 0° +
     * ((bg_end_angle + tolerance) - 360°) Here we can assume background is not near 0° because of the first two initial
     * checks */
    else if ((360U < (bigger_angle + tolerance_deg)) && (angle <= 0U + ((bigger_angle + tolerance_deg) - 360U))
             && (angle > bigger_angle)) {
