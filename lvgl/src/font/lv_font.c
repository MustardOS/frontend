 ...
 */
int lv_font_get_glyph_dsc(
    const lv_font_t *font_p, lv_font_glyph_dsc_t *dsc_out, uint32_t letter, uint32_t letter_next
) {
    ...
    while (f) {
        int found = f->get_glyph_dsc(f, dsc_out, letter, letter_next);
        if (found) {
