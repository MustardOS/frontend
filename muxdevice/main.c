...
void init_elements() {
    ...
    if (strcasecmp(device.DEVICE.NAME, "RG28XX") == 0) {
    ...
    if (strlen(overlay) > 0 && theme.MISC.IMAGE_OVERLAY) {
        lv_obj_t * overlay_img = lv_img_create(ui_scrDevice);
        lv_img_set_src(overlay_img, overlay);
        lv_obj_move_foreground(overlay_img);
    }

    if (TEST_IMAGE) display_testing_message(ui_scrDevice);
}
    ...
    if (nav_moved) {
        if (lv_group_get_obj_count(ui_group) > 0) {
            ...
            snprintf(static_image, sizeof(static_image), "%s",
                     load_static_image(ui_scrDevice, ui_group));
...
int main(int argc, char *argv[]) {
    ...
    lv_obj_set_user_data(ui_scrDevice, basename(argv[0]));
    ...
    current_wall = load_wallpaper(ui_scrDevice, NULL, theme.MISC.ANIMATED_BACKGROUND);
    ...
    lv_label_set_text(ui_lblMessage, current_wall);
