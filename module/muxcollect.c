...
static void image_refresh(const char *image_type) {
    ...
    if (strcasecmp(image_type, "box") == 0) {
        ...
        if (resolve_save_screenshot(items[current_item_index].extra_data, 1, save_image, sizeof(save_image))) {
...
static void show_module_help(void) {
    ...
    set_preview_hint(0);
    show_info_box(lang.muxcollect.title, lang.muxcollect.help, 0);
}
...
static void show_actions_dialog(void) {
    ...
    const int is_folder = has_items && items[current_item_index].content_type == content_type_folder;
...
int muxcollect_main(const int add, const char *dir, const int last_index) {
    ...
    if (ui_count_static > 0) {
        ...
        if (tag_filtered)
            ...
            empty_state_show(lang.muxcollect.none, lang.muxcollect.none_hint);
        clear_box_image();
    }
