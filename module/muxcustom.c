...
void show_help(lv_obj_t *element_focused) {
    struct help_msg help_messages[] = {
            ...
            {ui_lblTheme,     lang.MUXCUSTOM.THEME_ALTERNATE},
...
void init_navigation_group() {
    char option_file_path[MAX_BUFFER_SIZE];
    snprintf(option_file_path, sizeof(option_file_path), "%s/options.txt", STORAGE_THEME);
    int has_options_file = file_exist(option_file_path);
    ...
    apply_theme_list_item(&theme, ui_lblThemeOption, lang.MUXCUSTOM.THEME_ALTERNATE);
    ...
    apply_theme_list_glyph(&theme, ui_icoThemeOption, mux_module, "themeoption");
    ...
    apply_theme_list_drop_down(&theme, ui_droThemeOption, has_options_file ? read_text_from_file(option_file_path) : "");
    ...
    if (!has_options_file) {
        lv_obj_add_flag(ui_pnlThemeOption, LV_OBJ_FLAG_HIDDEN);
        ui_count--;
    }
}
...
void restore_options() {
    snprintf(theme_alt_original, sizeof(theme_alt_original), "%s", read_line_from_file((STORAGE_THEME "/active.txt"),1));
    int32_t option_index = lv_dropdown_get_option_index(ui_droThemeOption, theme_alt_original);
    if (option_index > 0) 
        lv_dropdown_set_selected(ui_droThemeOption, option_index);
}

void save_options() {
    char theme_alt[MAX_BUFFER_SIZE];
    lv_dropdown_get_selected_str(ui_droThemeOption, theme_alt, sizeof(theme_alt));
