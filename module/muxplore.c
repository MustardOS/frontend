...
static void create_content_items(void) {
    ...
    for (int i = 0; i < dir_count; i++) {
        ...
        content_item *new_item =
            add_item(&items, &item_count, dir_names[i], friendly_folder_name, dir_paths[i], content_type_folder);
    ...
    order_load(sys_dir);
    order_prepare(items, item_count, sys_dir);
    order_apply(items, item_count);
    ...
    if (sys_index < 0 && file_exist(MUOS_HST_LOAD)) {
        ...
        if (last_name) {
            const int idx = get_item_index_by_name(items, item_count, last_name, content_type_item);
...
static void handle_a_hold(void) {
    if (msgbox_active || hold_call || more_active(&more_menu)) return;
    video_preview_cancel();
...
static void handle_nav_page_up(void) {
    if (more_active(&more_menu)) return;
    handle_list_nav_page_up();
}

static void handle_nav_page_down(void) {
    if (more_active(&more_menu)) return;
...
static void navigate_to_dir(const char *new_dir, const int restore_index) {
    ...
    const int collect_vis = ui_count_static > 0 && items[current_item_index].content_type == content_type_item;
...
int muxplore_main(const int index, char *dir) {
    ...
    mux_input_options input_opts = {
        ...
        .press_handler =
            {
                ...
                [mux_input_dpad_up] = handle_nav_up,
                [mux_input_dpad_down] = handle_nav_down,
                [mux_input_dpad_left] = handle_nav_left,
                [mux_input_dpad_right] = handle_nav_right,
                [mux_input_l1] = handle_nav_page_up,
                [mux_input_r1] = handle_nav_page_down,
        ...
        .hold_handler = {
            ...
            [mux_input_dpad_up] = handle_nav_up_hold,
            [mux_input_dpad_down] = handle_nav_down_hold,
            [mux_input_dpad_left] = handle_nav_left_hold,
            [mux_input_dpad_right] = handle_nav_right_hold,
            [mux_input_l1] = handle_nav_page_up,
            [mux_input_r1] = handle_nav_page_down,
            [mux_input_r2] = handle_random_select,
        }
