...
static void restore_theme_filter_options() {
    lv_dropdown_set_selected(ui_droAllThemes_themefilter, config.THEME_FILTER.ALL_THEMES);
    lv_dropdown_set_selected(ui_droGrid_themefilter, config.THEME_FILTER.GRID);
    lv_dropdown_set_selected(ui_droHdmi_themefilter, config.THEME_FILTER.HDMI);
    lv_dropdown_set_selected(ui_droLanguage_themefilter, config.THEME_FILTER.LANGUAGE);
}

static void save_theme_filter_options() {
    int is_modified = 0;

    CHECK_AND_SAVE_STD(themefilter, AllThemes, "themefilter/allthemes", INT, 0);
    CHECK_AND_SAVE_STD(themefilter, Grid, "themefilter/grid", INT, 0);
