...
#define INSTALL(NAME, ENUM, UDATA) { UDATA, lang.MUXINSTALL.HELP.ENUM },
            INSTALL_ELEMENTS
#undef INSTALL
    };
...
static void init_navigation_group_grid(char *item_labels[], char *item_grid_labels[], char *glyph_names[]) {
    ...
    for (int i = 0; i < UI_COUNT; i++) {
        ...
        load_element_image_specifics(mux_dim, mux_module, "grid", glyph_names[i], "default", "png", grid_img, sizeof(grid_img));
        ...
        load_element_image_specifics(mux_dim, mux_module, "grid", glyph_name_focused, "default_focused", "png", grid_img_foc, sizeof(grid_img_foc));
