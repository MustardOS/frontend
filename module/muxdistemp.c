...
#define DISTEMP(NAME, ENUM, UDATA) { UDATA, lang.MUXDISTEMP.HELP.ENUM },
            DISTEMP_ELEMENTS
#undef DISTEMP
    };
...
static char *generate_time_string(void) {
    char *buf = malloc(96 * 7);
    if (!buf) return NULL;

    char *ptr = buf;
    for (int i = 0; i < 96; i++) {
        ptr += sprintf(ptr, "%02d:%02d", i / 4, (i % 4) * 15);
        if (i < 95) *ptr++ = '\n';
    }
...
static void save_distemp_options(void) {
    ...
    if (is_modified > 0) {
        ...
        const char *sunrise_script[] = {OPT_PATH "script/init/async/S08sunrise.sh", "restart", NULL};
...
static void init_navigation_group(void) {
    ...
    char *time_values = generate_time_string();
    ...
    time_values = generate_time_string();
...
static void handle_x(void) {
    ...
    lv_obj_t *dro = (current_item_index == 0) ? ui_droSunriseTemp_distemp : ui_droSunsetTemp_distemp;
    int temp = (int) lv_dropdown_get_selected(dro) - 255;
