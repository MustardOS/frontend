...
static void image_refresh(char *image_type) {
    ...
    if (strlen(h_core_artwork) <= 1) {
        ...
        if (strcasecmp(image_type, "box") != 0 || !grid_mode_enabled || !config.VISUAL.BOX_ART_HIDE) {
            load_image_catalogue(h_core_artwork, h_file_name, "", "default", mux_dim, image_type,
                                 image, sizeof(image));
        }
...
static void process_load(int from_start) {
    ...
    if (strcmp(launch_path, resolved_path) != 0) {
        ...
        if (!ends_with(history_file, ".cfg")) strncat(history_file, ".cfg", sizeof(history_file) - strlen(history_file) - 1);
