...
static GLuint compile_shader(const GLenum type, const char **parts, const int count) {
...
int shader_load(const char *path, shader_prog_t *out) {
    ...
    const GLuint vs = compile_shader(GL_VERTEX_SHADER, vs_parts, 1);

    const char *fs_parts[] = {frag, frag_src};
    const GLuint fs = compile_shader(GL_FRAGMENT_SHADER, fs_parts, 2);
    ...
    const GLuint prog = link_program(vs, fs);
    glDeleteShader(vs);
    glDeleteShader(fs);
    if (!prog) return 0;
