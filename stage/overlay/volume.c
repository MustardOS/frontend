...
static int ensure_volume_path(const char *dim, int step) {
    ...
    volume_overlay_path[0] = '\0';

    char name[64];
    snprintf(name, sizeof(name), "volume_%d", step);

    if (load_stage_image("volume", ovl_go_cache.core, ovl_go_cache.system, name, dim, volume_overlay_path)) return 1;
