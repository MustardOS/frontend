...
void create_theme_items() {
    ...
    if (file_count > 0) {
        for (size_t i = 0; i < file_count; i++) {
            ...
            lv_obj_set_style_pad_left(ui_lblThemeItem, theme.FONT.LIST_PAD_LEFT, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_pad_right(ui_lblThemeItem, theme.FONT.LIST_PAD_RIGHT, LV_PART_MAIN | LV_STATE_DEFAULT);
                        ...
                        if (ev.value == 1) {
                            if (msgbox_active) {
                                ...
                                if (ev.code == device.RAW_INPUT.BUTTON.MENU_LONG) {
                                    ...
                                    write_text_to_file(MUOS_PDI_LOAD, "theme", "w");
                                    safe_quit = 1;
...
void init_elements() {
    ...
    if (strlen(overlay) > 0 && theme.MISC.IMAGE_OVERLAY) {
        lv_obj_t * overlay_img = lv_img_create(ui_scrTheme);
    ...
    if (nav_moved) {
        if (lv_group_get_obj_count(ui_group) > 0) {
            ...
            snprintf(new_wall, sizeof(new_wall), "%s", load_wallpaper(
                    ui_scrTheme, ui_group, theme.MISC.ANIMATED_BACKGROUND));
            ...
            snprintf(static_image, sizeof(static_image), "%s",
                     load_static_image(ui_scrTheme, ui_group));
